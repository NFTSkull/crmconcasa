...
export type MesaEvidenciaAsesorSectionProps = Readonly<{
  expedienteId: string;
}>;
...
/**
 * Solo lectura: Mesa consulta Evidencia del asesor (sin upload/reemplazo/eliminar).
 * Preview únicamente para MIME seguros (PDF/JPG/PNG/WEBP).
 */
export function MesaEvidenciaAsesorSection({
  expedienteId,
}: MesaEvidenciaAsesorSectionProps) {
    ...
    try {
      ...
      setError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : "No se pudo consultar la evidencia del asesor.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section
      aria-label="Evidencia del asesor"
    ...
    >
      ...
      {!loading && !error && !documento ? (
        <p className="mt-2 text-sm text-gray-700">
          El asesor no ha cargado evidencia.
        </p>
      ) : null}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {canPreview ? (
              <Button
                ...
                aria-label="Ver evidencia del asesor"
            ...
            <Button
              ...
              className="h-8 px-2.5 py-0 text-xs"
              disabled={archivoBusy}
              aria-label="Descargar evidencia del asesor"
