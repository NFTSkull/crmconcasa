...
export type MesaPagareUploadDialogMode = "upload" | "replace";

export type MesaPagareUploadDialogProps = Readonly<{
  open: boolean;
  mode: MesaPagareUploadDialogMode;
  fileName: string;
  mime: ClientePagareMime | string;
  fileSize: number;
  saving: boolean;
        ...
        <div className="space-y-3 px-4 py-4 text-sm text-gray-700">
          ...
          <dl className="rounded-lg border border-gray-100 bg-slate-50 px-3 py-2 text-xs">
            ...
            <div className="flex justify-between gap-2 py-1">
              ...
              <dd className="font-medium text-gray-900">{formatPagareMimeLabel(mime)}</dd>
            </div>
            <div className="flex justify-between gap-2 py-1">
