...
export type MesaSolicitudDocumentoUploadDialogMode = "upload" | "replace";

export type MesaSolicitudDocumentoUploadDialogProps = Readonly<{
  open: boolean;
  mode: MesaSolicitudDocumentoUploadDialogMode;
  fileName: string;
  mime: ClienteSolicitudMime | string;
  fileSize: number;
  saving: boolean;
...
export function MesaSolicitudDocumentoUploadDialog({
  open,
  mode,
  fileName,
  mime,
  ...
  onClose,
  onConfirm,
}: MesaSolicitudDocumentoUploadDialogProps) {
  const titleId = useId();
  ...
  const title = isReplace ? "Reemplazar Solicitud" : "Subir Solicitud";
  const confirmLabel = isReplace ? "Reemplazar Solicitud" : "Subir Solicitud";
    ...
    >
      ...
      >
        ...
        <div className="space-y-3 px-4 py-4 text-sm text-gray-700">
          {isReplace ? (
            <p>
              Se cargará una nueva versión del Solicitud. La versión vigente anterior se
          ...
          <dl className="rounded-lg border border-gray-100 bg-slate-50 px-3 py-2 text-xs">
            ...
            <div className="flex justify-between gap-2 py-1">
              ...
              <dd className="font-medium text-gray-900">{formatSolicitudDocumentoMimeLabel(mime)}</dd>
            </div>
            <div className="flex justify-between gap-2 py-1">
