...
import { MesaSolicitudDocumentoUploadDialog } from "@/components/mesa-control/MesaSolicitudDocumentoUploadDialog";
import {
...
export type MesaSolicitudDocumentoSectionProps = Readonly<{
  expedienteId: string;
...
export function MesaSolicitudDocumentoSection({
  expedienteId,
  etapaActual,
  puedeOperar,
  submittedToMesa = true,
}: MesaSolicitudDocumentoSectionProps) {
  ...
  const [selectedMime, setSelectedMime] = useState<ClienteSolicitudMime | null>(null);
  ...
  const operableWrite =
    puedeOperar &&
    submittedToMesa &&
    canMesaOperateSolicitudDocumento({ etapaActual, puedeOperar: true });

  const uiMode = resolveMesaSolicitudDocumentoUiMode({
    etapaActual,
    puedeOperar: operableWrite,
    hasDocumento: documento != null,
  });
  const writeEnabled = mesaSolicitudDocumentoWriteEnabled(uiMode) && operableWrite;

  const loadSolicitudDocumento = useCallback(async () => {
    setLoading(true);
    setLoadError(null);
    try {
      ...
      setLoadError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : "No se pudo cargar el Solicitud.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    void loadSolicitudDocumento();
  }, [loadSolicitudDocumento]);
  ...
  const applySelectedFile = (file: File) => {
    if (!writeEnabled || saving) return;
    const validation = validateClienteSolicitudFile(file);
    if (!validation.ok) {
    ...
    try {
      if (dialogMode === "replace") {
        await archivosRepo.replaceMesaDocumento({
          expedienteId,
          tipo_documento: CLIENTE_SOLICITUD_DOCUMENT_TIPO,
          file: selectedFile,
        });
      } else {
      ...
      setSuccessMsg(
        dialogMode === "replace"
          ? "Solicitud reemplazado correctamente."
          : "Solicitud cargado correctamente.",
      );
      setDialogOpen(false);
      setSelectedFile(null);
      setSelectedMime(null);
      setProgressLabel(null);
      await loadSolicitudDocumento();
      window.setTimeout(() => uploadButtonRef.current?.focus(), 0);
    } catch (err) {
      setProgressLabel(null);
      setWriteError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : "No se pudo registrar el Solicitud. Intenta de nuevo.",
      );
    } finally {
  ...
  const etapaLabel =
    ...
    etapaActual < CLIENTE_SOLICITUD_DOCUMENT_CONTRACT.etapaMinima
      ? "Disponible después de Inscripción"
      : null;

  return (
    <section aria-label="Solicitud" className="space-y-3 px-2 py-2 sm:px-3">
      ...
      {uiMode === "etapa_bloqueada" ? (
        <p className="text-sm text-gray-600">
          El Solicitud podrá cargarse después de concluir la inscripción.
        </p>
      ) : null}
      ...
      {!loading && !loadError && uiMode !== "etapa_bloqueada" && !documento ? (
        <p className="text-sm text-gray-600">Solicitud pendiente de carga.</p>
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {writeEnabled ? (
          <div className="w-full min-w-[14rem] max-w-md basis-full sm:basis-auto">
            <DocumentDropzone
              accept={CLIENTE_SOLICITUD_ACCEPT_ATTR}
              busy={saving}
              disabled={!writeEnabled || saving}
              selectedFileName={selectedFile?.name ?? null}
              aria-label={documento ? "Reemplazar Solicitud" : "Subir Solicitud"}
      ...
      {selectedFile && selectedMime ? (
        <MesaSolicitudDocumentoUploadDialog
          open={dialogOpen}
          mode={dialogMode}
