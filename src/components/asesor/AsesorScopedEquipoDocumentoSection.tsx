...
export type AsesorScopedEquipoDocumentoSectionProps = Readonly<{
  expedienteId: string;
...
/**
 * Sección dedicada opcional scoped por equipo (mismo patrón visual que Constancia SAT).
 * El montaje lo decide el padre vía `asesor_tipos_documento_visibles`.
 */
export function AsesorScopedEquipoDocumentoSection({
  expedienteId,
  tipo,
  label,
  uploadHint,
  ...
  const handleFiles = async (files: File[]) => {
    ...
    const validation = validateScopedEquipoPdfFile(file, label, maxBytes);
    if (!validation.ok) {
    ...
    try {
      ...
      await load();
      setSuccessMsg(documento?.id ? `${label} reemplazada.` : `${label} cargada.`);
      await onUploaded?.();
    } catch (err) {
      ...
      setWriteError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : `No se pudo guardar ${label}. Intenta de nuevo.`,
      );
    } finally {
      setSaving(false);
  ...
  return (
    ...
    >
      ...
      {!loading && !error && !documento ? (
        <p className="mt-2 text-sm text-gray-700">No se ha cargado {label}.</p>
      ) : null}
      ...
      {canUpload ? (
        <div className="mt-3 space-y-2">
          <DocumentDropzone
            accept={SCOPED_EQUIPO_PDF_ACCEPT_ATTR}
            busy={saving}
            disabled={saving}
            hint={uploadHint}
            aria-label={documento ? `Reemplazar ${label}` : `Subir ${label}`}
            selectedFileName={null}
            error={writeError}
            onFiles={(files) => void handleFiles(files)}
          />
          <p className="text-xs font-medium text-gray-800">
            {documento ? `Reemplazar ${label}` : `Subir ${label}`}
          </p>
        </div>
      ) : (
        <p className="mt-3 text-xs text-gray-500">
          {documento
            ? `El expediente está en solo lectura: puedes ver o descargar ${label}.`
            : `El expediente está en solo lectura: no se puede cargar ${label} ahora.`}
        </p>
      )}
