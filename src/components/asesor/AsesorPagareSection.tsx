...
export type AsesorPagareSectionProps = Readonly<{
  expedienteId: string;
 ...
 */
export function AsesorPagareSection({
  expedienteId,
  etapaActual,
}: AsesorPagareSectionProps) {
  ...
  const visible = shouldShowAsesorPagareSection(etapaActual);
    ...
    try {
      ...
      setError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : "No se pudo consultar el Pagaré.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <p className="mt-2 text-xs text-gray-500">Cargando Pagaré…</p>
      ) : null}
      ...
      {!loading && !error && !documento ? (
        <p className="mt-2 text-sm text-gray-700">
          Mesa Control todavía no ha cargado el Pagaré de este expediente.
        </p>
      ) : null}
      ...
      {!loading && documento ? (
        <>
          <p className="mt-2 text-sm text-gray-700">
            Mesa Control cargó el Pagaré correspondiente a este expediente.
          </p>
