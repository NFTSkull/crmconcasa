...
export type AsesorConstanciaSituacionFiscalSectionProps = Readonly<{
  expedienteId: string;
...
/**
 * Sección dedicada Constancia SAT (opcional).
 * No forma parte del checklist de obligatorios ni del progreso de envío.
 */
export function AsesorConstanciaSituacionFiscalSection({
  expedienteId,
  canUpload,
  onUploaded,
}: AsesorConstanciaSituacionFiscalSectionProps) {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    const validation = validateClienteConstanciaSituacionFiscalFile(file);
    if (!validation.ok) {
    ...
    try {
      ...
      setWriteError(
        err instanceof ExpedienteArchivosSupabaseError
          ? err.message
          : "No se pudo guardar Constancia SAT. Intenta de nuevo.",
      );
    } finally {
      setSaving(false);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <h3 className="text-sm font-semibold text-gray-900">Constancia SAT</h3>
      ...
      {!loading && !error && !documento ? (
        <p className="mt-2 text-sm text-gray-700">No se ha cargado Constancia SAT.</p>
      ) : null}
      ...
      {canUpload ? (
        <div className="mt-3 space-y-2">
          <DocumentDropzone
            accept={CLIENTE_CONSTANCIA_SITUACION_FISCAL_ACCEPT_ATTR}
            busy={saving}
            disabled={saving}
            hint={CLIENTE_CONSTANCIA_SITUACION_FISCAL_UPLOAD_HINT}
            aria-label={
              documento ? "Reemplazar Constancia SAT" : "Subir Constancia SAT"
            }
        ...
        <p className="mt-3 text-xs text-gray-500">
          {documento
            ? "El expediente está en solo lectura: puedes ver o descargar Constancia SAT."
            : "El expediente está en solo lectura: no se puede cargar Constancia SAT ahora."}
        </p>
      )}

      {saving ? (
        <p className="mt-2 text-xs text-gray-500" role="status">
          Guardando Constancia SAT…
        </p>
      ) : null}
      ...
      {preview ? (
        <MesaArchivoPreviewDialog
          ...
          onOpenInNewTab={() => {
            /* Constancia SAT insegura: no abrir en pestaña; solo modal para MIME seguros */
          }}
        />
      ) : null}
    </section>
