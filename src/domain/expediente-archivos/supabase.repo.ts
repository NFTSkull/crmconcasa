...
import { isInfonavitAutoGeneratedDocumentType } from "./infonavit-auto-document-types";
import type {
...
function buildResumenFromList(
  ...
  return TIPO_DOCUMENTO_CATALOGO.filter(
    (tipo) => !isInfonavitAutoGeneratedDocumentType(tipo),
  ).map((tipo) => {
...
export class SupabaseExpedienteArchivosRepo implements ExpedienteArchivosRepo {
  ...
  private async uploadOrReplace(params: UploadArchivoParams): Promise<void> {
    ...
    if (isInfonavitAutoGeneratedDocumentType(tipo)) {
      throw new ExpedienteArchivosSupabaseError(
        "Este documento lo genera el sistema. No se puede subir ni reemplazar.",
      );
    }
