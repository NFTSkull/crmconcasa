/**
 * Pipeline puro del worker shadow P3 (testeable sin Deno/Edge).
 * Cero OCR. Cero llamadas externas. Cero autofill.
 */
...
export type WorkerDeps = Readonly<{
  ...
  /** Opcional: P3 puede no-op (shadow no lee bytes). */
...
function safeLog(
  ...
  if (!log) return;
  // Solo keys permitidas (sin PII / path completo / bytes)
...
export async function processClaimedExtractionJob(
  ...
  if (!meta.ok) {
    const code = (meta.error_code ?? "document_not_found") as DocumentExtractionErrorCode;
    const retryable = isRetryableDocumentExtractionError(code);
    await deps.markFailed({
      jobId: job.job_id,
      errorCode: code === "document_not_found" ? "document_not_found" : "internal_error",
      retryable,
      leaseClaimedAt: job.claimed_at,
    });
    return {
  ...
  if (!completed.ok) {
    if (completed.error_code === "document_not_current" || completed.status === "stale") {
      return {
        job_id: job.job_id,
        outcome: "stale",
        error_code: "document_not_current",
    ...
    const code = (completed.error_code ?? "complete_conflict") as DocumentExtractionErrorCode;
    return {
      job_id: job.job_id,
      outcome: "failed",
      error_code: code,
