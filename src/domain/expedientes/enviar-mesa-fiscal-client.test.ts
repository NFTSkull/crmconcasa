...
describe("mapEnviarMesaFiscalHttpError", () => {
  ...
  it("REVISION_MANUAL / CTA reintentar validación", () => {
    const err = mapEnviarMesaFiscalHttpError({
      ok: false,
      status: "revision_manual",
      code: "SAT_WORKER_NOT_CONFIGURED",
      cta: "reintentar_validacion",
    });
    assert.match(err.message, /validación fiscal del SAT/i);
    assert.equal(err.cta, "reintentar_validacion");
  });
});
