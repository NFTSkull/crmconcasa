...
## 2026-08-26 - HOTFIX prod: live-sync biométricos timeout (P208 fail-closed)

Síntoma: asesor ve «No pudimos verificar el cupo en Google Sheets…» al agendar biométricos (Monterrey/Apodaca). Causa: `agenda-sheet-live-sync` modo single-date invocaba `listSheets()` completo en cada request FE (~150s medidos vía pg_net); `supabase.functions.invoke` expira antes → `invokeAgendaSheetLiveSync` devuelve `null` → `shouldBlockBookWithoutLiveSync` fail-closed. Edge/cron/Google/worker secret OK; horizon cron ~80ms. Fix: resolver pestaña vía `GOOGLE_SHEETS_TAB_MAP_JSON` + `resolveSheetTabForDate` (1× `getValues`); fallback `listSheets` solo si falta mapa. Post-deploy: ~11s, `fresh=true`, cap 15 intacto. 0 bookings / 0 expedientes tocados. P211 no implicado.

## 2026-08-26 - P211: vigencia documental 45 días (tramo 3–8 → release @9)

Causa: documentos de domicilio/EDC deben renovarse si el expediente supera 45 días naturales (Monterrey) desde la entrada al tramo operativo 3–8, no solo mientras está en etapa 3. Decisión: columnas `vigencia_documental_started_at` / `liberada_at` / `reingreso_completado_at`; started en transición desde &lt;3; **no reset** interno 3↔8; release sticky en primera llegada ≥9; trigger **solo clock/release/audit** (sin GUC spoofable); hard-gate en RPCs canónicos (`book_biometricos` antes P208, sheet, avanzar, retención, repair, apply); Mesa override = `mesa_mover` no llama assert. Docs frescos por `created_at` local ≥ día 46 sin validación Mesa. UI detalle asesor. Cert final: override A–F, T15/T16, T47–T52, concurrencia 2 conn, retención atómica. **Cloud apply 211** vía `db query --linked -f` + `schema_migrations` 211; cohort 187 / >45=2 / started_null=0 / liberada>=9=173/173; GUC ausente; business workflow writes=0.
...
## 2026-08-18 - feat LOCAL: clasificar Cambios por revisar (mig 193)

Causa: el chip P192 mezclaba corrección solicitada por Mesa con actualización espontánea del asesor. Decisión: read-model STABLE `mesa_cambio_revision_clasificacion` (origins REQUESTED_CORRECTION / ADVISOR_UPDATE / AMBIGUOUS / LEGACY) sobre el mismo universo P192. Ventana = latest lote `pendiente_revision`+`submitted_at`; cycle_start = prior.reviewed_at / prior.submitted_at / fecha_envio_mesa; submitted_at < envío sin prior → AMBIGUOUS; sin lote P130 → LEGACY. Solicitudes canónicas abiertas: DG (`cliente_datos.revision.update` rechazado sin validado posterior), documental (`documento_revisiones` rechazado cerrado por mismo `tipo_documento`), rechazo operativo (sin reactivación previa al lote). `mesa_bandeja_categoria_resumen` intacto. RPC misma firma; `p_quick_filter` `correccion_solicitada` / `otras_actualizaciones`. UI: chip «Cambios por revisar» + subfiltros. 0 writes. Operación de citas → **195**.

## 2026-08-18 - hotfix LOCAL: correcciones P130 pendientes en chip Mesa (mig 192)

Causa: `mesa_bandeja_categoria_resumen` (P102) solo miraba pack legado `ine|estado_cuenta|nss|direccion` y timestamp de `cliente_datos`. Las correcciones reales P130 viven en `expediente_asesor_cambio_lotes.status=pendiente_revision` (docs `cliente_*` suelen quedar `subido`, no `resubido`). El chip «Correcciones listas para revisar» y el dashboard asesor «Corrección enviada» no las veían. Decisión: helper canónico `expediente_tiene_correccion_asesor_pendiente` (submitted_at NOT NULL); precedencia P130 pending sobre `correccion_requerida`/faltantes; legacy P102/P167 intacta si no hay lote pendiente. `mesa_list_bandeja_page` no duplica lógica. Sort: latest pending `submitted_at`. Marcar revisado sigue igual (sale del predicado al pasar a `revisado`). Sin backfill, sin stage gate, sin Cloud/push/PR. Operación de citas → **195**.

## 2026-08-18 - hotfix LOCAL: asesor tareas accionables excluyen terminales (mig 191)

Causa: `asesor_inbox_pendiente_agendar_biometricos` (P167) solo miraba `submitted_to_mesa` + `etapa_actual` + booking vigente. Un expediente `ciclo_estado=cancelado` / `resultado_real=cancelado` en etapa 3 sin booking entraba al chip «Agendar biométricos» y al contador. Decisión: helper canónico `asesor_inbox_es_accionable` reutiliza `asesor_inbox_resultado_real`; `cancelado` y `rechazado_mesa` no son tarea. Los tres `pendiente_*` (bio/firma/acuse) comparten el guard; list+summary heredan. Corrección documental no es terminal. Reingreso intacto (si el estado actual vuelve a `en_tramite`, reaparece). Mig **191**; Operación de citas pausada → **193** (192 = hotfix correcciones P130). Sin Cloud/push/PR.
...
## 2026-08-05 - Admin UX B2: tabla simple, drawer y divulgación progresiva

B2 (solo UX/UI) sobre B1: tabla Mesa reducida a 6 columnas; detalle secundario (envío, espera, corrección, rechazo, timeline) en drawer lateral accesible (Escape, clic exterior, focus trap, retorno de foco) con tabs Resumen/Seguimiento/Precal — reutiliza `openTimeline`/`getExpedienteMesaTimeline` sin consultas nuevas; precal solo si ya está en `precalItems`. Producción: una fila expandida a la vez + «Ver expedientes» → `applyAsesorFilter` + tab Expedientes. Reportes/Ingresos: KPIs visibles, tablas en `AdminCollapsibleSection` colapsado. Badges de situación mapean tonos a señales existentes. Tests: `adminStatusBadge.test.ts` + `page.admin-tabs-b2.mount.test.ts`. Sin backend/RPC/RLS.

## 2026-08-05 - Admin UX B1: reorganización visual en 4 pestañas

Problema: `/admin` acumulaba reporte histórico, ingresos, filtros, KPIs, snapshot, Mesa, producción y precal en un solo scroll. B1 (solo UX/UI): pestañas Resumen/Expedientes/Reportes/Producción con `AdminTabs` (WAI-ARIA, roving tabindex, flechas/Home/End) + helpers puros `adminUxTabs.ts`. Decisiones: (1) paneles inactivos montados con `hidden` → estado/datos intactos y cero consultas extra; (2) barra de filtros global visible en Resumen/Expedientes/Producción y oculta en Reportes (tiene filtros propios) con microcopy de alcance; (3) cohorte comparte subtab con histórico («Histórico y cohorte») porque se calcula dentro de `AdminReporteExpedientesSection` con los mismos filtros — separarlo exigiría tocar consultas (fuera de B1); (4) producción con filtro de etapa activo ya no desaparece: muestra aviso + «Quitar filtro de etapa» (misma regla, mejor visibilidad); (5) clic en tarjeta de etapa cambia a Expedientes y enfoca la tabla (conserva el comportamiento de filtro+focus); (6) `?adminTab=` vía `history.replaceState`, sin router ni rutas nuevas. Sin cambios de dominio/repos/RPC/RLS. Tests: `adminUxTabs.test.ts` + `page.admin-tabs-b1.mount.test.ts` (21 casos).
