...
- **fix/asesor-busqueda-nss — P088 (local):** buscador de `/asesor` corregido: un término sin dígitos ya no coincide con todos los expedientes (bug `nssDigits.includes("")` === `true`); NSS funciona completo, parcial y con espacios/guiones; nombre, teléfono y programa sin regresiones. Lógica extraída a `src/domain/expedientes/asesor-list-search.ts` (`matchesAsesorSearch`, comparación numérica solo con `digitsTerm.length > 0`); búsqueda antes de paginar; filtros, KPIs, orden y paginación intactos; sin SQL/Cloud/commit/push/deploy.
