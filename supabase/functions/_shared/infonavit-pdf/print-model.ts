...
function personaFromParts(p: {
  ...
  const composed = formatNombreCompleto({ nombres, apellidoPaterno, apellidoMaterno, nombreCompleto: p.nombreCompleto });
...
function mapRef(r: InfonavitReferenciaInput | undefined): InfonavitReferenciaPrint {
  ...
  return { ...p, lada: blankable(r?.lada), telefono: blankable(r?.telefono), celular: blankable(r?.celular) };
}

function mapBen(b: InfonavitBeneficiarioInput | null | undefined): InfonavitPersonaPrint & { parentesco: string } {
  return { ...personaFromParts(b ?? {}), parentesco: blankable(b?.parentesco) };
}
...
export function composeDireccionLibre(snapshot: InfonavitPdfSnapshotInput): string {
  ...
  const parts = [
    ...
    blankable(v?.colonia) ? `COL. ${blankable(v?.colonia, { collapseSpaces: true })}` : "",
...
export function formatMontoPrint(amount: number | null | undefined): string {
  if (amount === null || amount === undefined) return "";
  return formatMoneyMx(amount, { withSymbol: false });
}

export function formatPlazoPrint(cred: InfonavitPdfSnapshotInput["credito"]): string {
  const plazo = cred?.plazoAnios === null || cred?.plazoAnios === undefined ? cred?.plazoMeses : cred.plazoAnios;
...
export function buildInfonavitPrintModel(snapshot: InfonavitPdfSnapshotInput): InfonavitPrintModel {
  ...
  const propuestaLines = propuesta.split(/\n/).map((l) => l.trim()).filter((l) => l.length > 0);
  ...
  return {
    ...
    ciudadCierre: blankable(snapshot.ciudadCierre || snapshot.localidad, { collapseSpaces: true }),
    ...
    nss: blankable(c.nss), curp: blankable(c.curp), rfc: blankable(c.rfc), celular: blankable(c.celular), correo: blankable(c.correo),
    telefono: blankable(c.telefono), ladaTelefono: blankable(c.ladaTelefono),
    identificacionTipo: blankable(id?.tipo), identificacionNumero: blankable(id?.numero), identificacionVigencia: blankable(id?.vigencia),
    empresaNombre: blankable(emp?.nombre), registroPatronal: blankable(emp?.registroPatronal), empresaLada: blankable(emp?.lada), empresaTelefono: blankable(emp?.telefono), empresaExtension: blankable(emp?.extension),
