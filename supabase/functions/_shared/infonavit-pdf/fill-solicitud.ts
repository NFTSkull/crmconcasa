/**
 * Fill Solicitud Inscripción Crédito — reset defaults + mapping.
 * Género: C1=M, C0=F. Promotor/No. crédito Infonavit permanecen en blanco.
 * T31–T33 (destino de recursos) son editables desde Mesa en mapping v3.
 * No flatten aquí.
 */
...
export async function fillSolicitud(args: {
  ...
  const destino = snapshot.destinoRecursos;
  ...
  setTextValue(form, SOLICITUD_FIELD.T3_AP_PATERNO, blankable(c.apellidoPaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T4_AP_MATERNO, blankable(c.apellidoMaterno), FONT_SIZE);
  ...
  setTextValue(form, SOLICITUD_FIELD.T8_VIGENCIA, blankable(id?.vigencia), FONT_SIZE);
  ...
  // Empresa y NRPP: mapping directo del snapshot (Datos Generales / edición Mesa).
  setTextValue(form, SOLICITUD_FIELD.T13_EMPRESA, blankable(emp?.nombre), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T14_REG_PATRONAL, blankable(emp?.registroPatronal), FONT_SIZE);
  ...
  setTextValue(
    ...
    blankable(viv?.calle) || blankable(viv?.direccionCompleta, { collapseSpaces: true }),
    FONT_SIZE,
  );
  ...
  setTextValue(form, SOLICITUD_FIELD.T25_MUNICIPIO, blankable(viv?.municipio), FONT_SIZE);
  ...
  setExclusiveCheck(
    form,
    [SOLICITUD_FIELD.C8_PROP_PROPIA, SOLICITUD_FIELD.C4_PROP_CONYUGE, SOLICITUD_FIELD.C5_PROP_FAMILIAR],
    propActive,
  );
  ...
  // 4.1 Destino de los recursos (antes se forzaba en blanco).
  const porcentajeTitulacion = destino?.porcentajeTitulacion;
  setTextValue(
    form,
    SOLICITUD_FIELD.T31_BLANK,
    porcentajeTitulacion === null || porcentajeTitulacion === undefined
      ? ""
      : blankable(String(porcentajeTitulacion)),
    FONT_SIZE,
  );
  setTextValue(form, SOLICITUD_FIELD.T32_BLANK, blankable(destino?.clabeNotaria), FONT_SIZE);
  setTextValue(
    form,
    SOLICITUD_FIELD.T33_BLANK,
    blankable(destino?.clabeDerechohabiente),
    FONT_SIZE,
  );

  setTextValue(form, SOLICITUD_FIELD.T30_REF1_AP_PAT, blankable(ref1?.apellidoPaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T34_REF1_AP_MAT, blankable(ref1?.apellidoMaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T35_REF1_NOMBRES, blankable(ref1?.nombres), FONT_SIZE);
  ...
  setTextValue(form, SOLICITUD_FIELD.T39_REF2_AP_PAT, blankable(ref2?.apellidoPaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T40_REF2_AP_MAT, blankable(ref2?.apellidoMaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T41_REF2_NOMBRES, blankable(ref2?.nombres), FONT_SIZE);
  ...
  // Beneficiario: parentesco y nombre legal separados explícitamente.
  setTextValue(form, SOLICITUD_FIELD.T45_BEN_PARENTESCO, blankable(ben?.parentesco), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T46_BEN_AP_PAT, blankable(ben?.apellidoPaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T47_BEN_AP_MAT, blankable(ben?.apellidoMaterno), FONT_SIZE);
  setTextValue(form, SOLICITUD_FIELD.T48_BEN_NOMBRES, blankable(ben?.nombres), FONT_SIZE);

  setTextValue(
    form,
    SOLICITUD_FIELD.T56_CIUDAD,
    blankable(snapshot.ciudadCierre || snapshot.localidad, { collapseSpaces: true }),
    FONT_SIZE,
  );
