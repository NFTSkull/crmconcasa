...
export async function buildSolicitudDocx(
  ...
  const children: FileChild[] = [
    ...
    labeledField("Monto de crédito solicitado ($)", model.montoCreditoSolicitado),
    spacer(),
    labeledField("Plazo solicitado (años, máximo 10)", model.plazo),
    spacer(),
    sectionTitle("4.1 DESTINO DE LOS RECURSOS"),
    labeledField("% para titulación (máximo 30)", model.porcentajeTitulacion),
    spacer(),
    labeledField("CLABE de la notaría", model.clabeNotaria),
    spacer(),
    labeledField("CLABE del derechohabiente", model.clabeDerechohabiente),
    spacer(),
