-- Alta NSS-only: wrapper exclusivo de asesores externos.
...
BEGIN
  ...
  SELECT public.create_expediente_externo_nss('99765432109') INTO v_res;
  v_exp := (v_res->>'id')::uuid;
  PERFORM public.__ext_nss_reset_auth();

  PERFORM public.__ext_nss_assert(v_exp IS NOT NULL, 'alta externa devuelve id');
  ...
  PERFORM public.__ext_nss_set_auth(v_ext);
  BEGIN
    PERFORM public.create_expediente_externo_nss('123');
  EXCEPTION WHEN SQLSTATE '22023' THEN
    v_bad_nss := true;
  END;
  PERFORM public.__ext_nss_reset_auth();
  PERFORM public.__ext_nss_assert(v_bad_nss, 'NSS inválido conserva gate de create_expediente');

  IF v_int IS NOT NULL THEN
    PERFORM public.__ext_nss_set_auth(v_int);
    BEGIN
      PERFORM public.create_expediente_externo_nss('99765432108');
    EXCEPTION WHEN SQLSTATE '42501' THEN
      v_denied := true;
    END;
    PERFORM public.__ext_nss_reset_auth();
    PERFORM public.__ext_nss_assert(v_denied, 'asesor interno no puede usar wrapper externo');
  END IF;
