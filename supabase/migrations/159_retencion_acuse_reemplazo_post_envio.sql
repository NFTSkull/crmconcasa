-- ConCasa CRM — Hotfix Acuse: reemplazo post-envío por asesor dueño
-- Permite reemplazar el documento principal activo (subido/resubido/rechazado/validado)
-- sin exigir corrección Mesa. Soft-delete de la versión anterior; no toca opción A/B,
-- etapa/subestado (si ya ≥9), retencion_envios, ni bookings de firmas.
-- No edita migraciones 001–158. Número 159 (evita colisión con otras migraciones 158_* en ramas paralelas).
...
COMMENT ON FUNCTION public.register_expediente_documento_retencion(uuid, text, text, text, text, bigint) IS
  'P132/P117 + hotfix Acuse: registra retención; principal en etapa 8 avanza 8→9; reemplazo post-envío (etapa ≥8) soft-delete previa sin exigir rechazo Mesa; no altera bookings.';
