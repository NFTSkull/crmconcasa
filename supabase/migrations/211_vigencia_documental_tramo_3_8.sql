-- ConCasa CRM — P211: vigencia documental 45 días (tramo etapas 3–8).
-- Release sticky al llegar a etapa >= 9. Cloud max previo = 210.
-- NO cron. NO reingreso post-biométricos. NO validación Mesa para docs fresh.

-- =============================================================================
-- 1) Columnas
-- =============================================================================
ALTER TABLE public.expedientes
  ADD COLUMN IF NOT EXISTS vigencia_documental_started_at TIMESTAMPTZ NULL,
  ADD COLUMN IF NOT EXISTS vigencia_documental_liberada_at TIMESTAMPTZ NULL,
  ADD COLUMN IF NOT EXISTS vigencia_reingreso_completado_at TIMESTAMPTZ NULL;

COMMENT ON COLUMN public.expedientes.vigencia_documental_started_at IS
  'P211: inicio del episodio actual del tramo operativo 3–8 (no reset interno).';
COMMENT ON COLUMN public.expedientes.vigencia_documental_liberada_at IS
  'P211: primera llegada a etapa >= 9; sticky (no se borra en 9→8).';
COMMENT ON COLUMN public.expedientes.vigencia_reingreso_completado_at IS
  'P211: auditoría — primer avance exitoso tras vencimiento con docs frescos.';

-- =============================================================================
-- 2) RO helper
-- =============================================================================
CREATE OR REPLACE FUNCTION public.expediente_vigencia_documental_estado(
...
DECLARE
  ...
  v_tracking_unknown BOOLEAN := FALSE;
  v_dom RECORD;
  v_edc RECORD;
  v_dom_fresco BOOLEAN := FALSE;
  v_edc_fresco BOOLEAN := FALSE;
  v_docs_ok BOOLEAN := FALSE;
  v_reingreso_req BOOLEAN := FALSE;
  v_listo BOOLEAN := FALSE;
  v_restantes INT;
  v_ultimo_dia DATE;
  v_primer_vencido DATE;
  v_reason TEXT := NULL;
BEGIN
  ...
  SELECT
    ...
    e.submitted_to_mesa,
    e.deleted_at,
    e.vigencia_documental_started_at,
    e.vigencia_documental_liberada_at,
    e.vigencia_reingreso_completado_at
  INTO v_exp
  ...
  v_today := (clock_timestamp() AT TIME ZONE 'America/Monterrey')::date;

  v_applicable := (
    v_exp.deleted_at IS NULL
    AND v_exp.submitted_to_mesa IS TRUE
    AND v_exp.ciclo_estado = 'activo'
    AND v_exp.etapa_actual BETWEEN 3 AND 8
    AND v_exp.vigencia_documental_liberada_at IS NULL
  );

  IF NOT v_applicable THEN
    IF v_exp.vigencia_documental_liberada_at IS NOT NULL THEN
      v_reason := 'already_released';
    ELSIF v_exp.etapa_actual >= 9 THEN
      v_reason := 'stage_gte_9';
    ELSIF v_exp.etapa_actual < 3 THEN
      v_reason := 'stage_lt_3';
    ELSIF v_exp.ciclo_estado IS DISTINCT FROM 'activo' THEN
      v_reason := 'ciclo_no_activo';
    ELSIF v_exp.submitted_to_mesa IS DISTINCT FROM TRUE THEN
      v_reason := 'not_submitted';
    ELSE
    ...
    RETURN jsonb_build_object(
      'applicable', false,
      'reason', v_reason,
      'started_at', v_exp.vigencia_documental_started_at,
      'liberada_at', v_exp.vigencia_documental_liberada_at,
      'started_date', NULL,
      'ultimo_dia_vigente', NULL,
      'dias_transcurridos', NULL,
      'limite_dias', v_limite,
      'dias_restantes', NULL,
      'vencido', false,
      'tracking_unknown', false,
      'comprobante_documento_id', NULL,
      'comprobante_fresco', false,
      'estado_cuenta_documento_id', NULL,
      'estado_cuenta_fresco', false,
      'docs_frescos_completos', false,
      'reingreso_requerido', false,
      'listo_para_continuar', true,
      'reingreso_completado_at', v_exp.vigencia_reingreso_completado_at,
      'blocking_reason', NULL
    );
  END IF;

  IF v_exp.vigencia_documental_started_at IS NULL THEN
    v_tracking_unknown := TRUE;
    RETURN jsonb_build_object(
      'applicable', true,
      'reason', 'tracking_unknown',
      'started_at', NULL,
      'liberada_at', NULL,
      'started_date', NULL,
      'ultimo_dia_vigente', NULL,
  ...
  v_started_date := (v_exp.vigencia_documental_started_at AT TIME ZONE 'America/Monterrey')::date;
  v_dias := v_today - v_started_date;
  v_ultimo_dia := v_started_date + v_limite;
  v_primer_vencido := v_started_date + v_limite + 1;
  v_vencido := v_dias > v_limite;
  v_restantes := GREATEST(v_limite - v_dias, 0);

  SELECT d.id, d.created_at
  INTO v_dom
  FROM public.expediente_documentos d
  WHERE d.expediente_id = p_expediente_id
    AND d.tipo_documento = 'cliente_comprobante_domicilio'
  ...
  IF v_dom.id IS NOT NULL THEN
    v_dom_fresco := (
      (v_dom.created_at AT TIME ZONE 'America/Monterrey')::date >= v_primer_vencido
    );
  END IF;
  IF v_edc.id IS NOT NULL THEN
    v_edc_fresco := (
      (v_edc.created_at AT TIME ZONE 'America/Monterrey')::date >= v_primer_vencido
    );
  END IF;

  v_docs_ok := v_dom_fresco AND v_edc_fresco;
  v_reingreso_req := v_vencido AND NOT v_docs_ok;
  v_listo := (NOT v_vencido) OR v_docs_ok;

  IF v_reingreso_req THEN
    v_reason := 'VIGENCIA_DOCUMENTAL_REINGRESO_REQUERIDO';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'started_at', v_exp.vigencia_documental_started_at,
    'liberada_at', v_exp.vigencia_documental_liberada_at,
    'started_date', v_started_date,
    'ultimo_dia_vigente', v_ultimo_dia,
    'dias_transcurridos', v_dias,
    'limite_dias', v_limite,
    'dias_restantes', v_restantes,
    'vencido', v_vencido,
    'tracking_unknown', false,
    'comprobante_documento_id', v_dom.id,
    'comprobante_fresco', v_dom_fresco,
    'estado_cuenta_documento_id', v_edc.id,
    'estado_cuenta_fresco', v_edc_fresco,
    'docs_frescos_completos', v_docs_ok,
    'reingreso_requerido', v_reingreso_req,
    'listo_para_continuar', v_listo,
    'reingreso_completado_at', v_exp.vigencia_reingreso_completado_at,
    'blocking_reason', CASE
      WHEN v_reingreso_req THEN 'VIGENCIA_DOCUMENTAL_REINGRESO_REQUERIDO'
      ELSE NULL
    END,
    'missing_comprobante', v_vencido AND NOT v_dom_fresco,
    'missing_estado_cuenta', v_vencido AND NOT v_edc_fresco
  );
END;
$$;

REVOKE ALL ON FUNCTION public.expediente_vigencia_documental_estado(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.expediente_vigencia_documental_estado(UUID) TO authenticated;

COMMENT ON FUNCTION public.expediente_vigencia_documental_estado(UUID) IS
  'P211: read-model vigencia documental tramo 3–8 (días Monterrey + docs frescos).';

-- =============================================================================
-- 3) Assert
-- =============================================================================
CREATE OR REPLACE FUNCTION public.assert_expediente_vigencia_documental_ok(
  p_expediente_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
DECLARE
  v_estado JSONB;
BEGIN
  v_estado := public.expediente_vigencia_documental_estado(p_expediente_id);
  IF v_estado IS NULL THEN
    RAISE EXCEPTION 'assert_expediente_vigencia_documental_ok: expediente no encontrado'
      USING ERRCODE = 'P0002';
  END IF;

  IF coalesce((v_estado->>'applicable')::boolean, false) IS NOT TRUE THEN
    RETURN v_estado;
  END IF;

  IF coalesce((v_estado->>'tracking_unknown')::boolean, false) THEN
    RETURN v_estado;
  END IF;

  IF coalesce((v_estado->>'vencido')::boolean, false) IS NOT TRUE THEN
    RETURN v_estado;
  END IF;

  IF coalesce((v_estado->>'docs_frescos_completos')::boolean, false) THEN
    RETURN v_estado;
  END IF;

  RAISE EXCEPTION 'VIGENCIA_DOCUMENTAL_REINGRESO_REQUERIDO'
    USING ERRCODE = 'P0001',
      DETAIL = jsonb_build_object(
        'missing_comprobante', coalesce((v_estado->>'missing_comprobante')::boolean, false),
        'missing_estado_cuenta', coalesce((v_estado->>'missing_estado_cuenta')::boolean, false),
        'dias_transcurridos', v_estado->'dias_transcurridos'
      )::text;
END;
$$;

REVOKE ALL ON FUNCTION public.assert_expediente_vigencia_documental_ok(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.assert_expediente_vigencia_documental_ok(UUID) TO authenticated;

COMMENT ON FUNCTION public.assert_expediente_vigencia_documental_ok(UUID) IS
  'P211: hard gate avance canónico; skip si no applicable / tracking_unknown / vigente / docs frescos.';

-- =============================================================================
-- 4) Trigger central: clock + release ONLY (NO assert / NO GUC)
-- Hard-gate P211 vive en RPCs canónicos (book/avanzar/retención/sheets).
-- mesa_mover_etapa_operativa NO llama assert → override sin GUC spoofable.
-- =============================================================================
CREATE OR REPLACE FUNCTION public.expedientes_vigencia_documental_biu()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- A) Entrada a tramo desde <3 → nuevo episodio (reset started_at)
  IF coalesce(OLD.etapa_actual, 0) < 3
     AND NEW.etapa_actual BETWEEN 3 AND 8
     AND NEW.vigencia_documental_liberada_at IS NULL THEN
    NEW.vigencia_documental_started_at := clock_timestamp();
  END IF;

  -- B) Movimientos internos 3–8: no tocar started_at

  -- C) Primera llegada a >=9 → release sticky
  IF NEW.etapa_actual >= 9
     AND coalesce(OLD.etapa_actual, 0) < 9 THEN
    NEW.vigencia_documental_liberada_at := coalesce(
      NEW.vigencia_documental_liberada_at,
      clock_timestamp()
    );
  END IF;

  -- D) Nunca borrar liberada_at
  IF OLD.vigencia_documental_liberada_at IS NOT NULL THEN
    NEW.vigencia_documental_liberada_at := OLD.vigencia_documental_liberada_at;
  END IF;

  -- E) Auditoría reingreso_completado (sin RAISE): primer avance con vencido+fresh
  IF OLD.etapa_actual BETWEEN 3 AND 8
     AND NEW.etapa_actual > OLD.etapa_actual
     AND NEW.etapa_actual <= 9
     AND NEW.vigencia_documental_liberada_at IS NULL
     AND NEW.vigencia_reingreso_completado_at IS NULL THEN
    v_estado := public.expediente_vigencia_documental_estado(NEW.id);
    IF coalesce((v_estado->>'vencido')::boolean, false)
       AND coalesce((v_estado->>'docs_frescos_completos')::boolean, false) THEN
      NEW.vigencia_reingreso_completado_at := clock_timestamp();
    END IF;
  END IF;
...
CREATE TRIGGER trg_expedientes_vigencia_documental_biu
  BEFORE UPDATE ON public.expedientes
  FOR EACH ROW
  EXECUTE FUNCTION public.expedientes_vigencia_documental_biu();

COMMENT ON FUNCTION public.expedientes_vigencia_documental_biu() IS
  'P211: clock/release/audit only. Hard-gate en RPCs canónicos; Mesa override = no assert (sin GUC).';

-- =============================================================================
-- 5) Backfill started_at (episodio actual del tramo 3–8)
-- =============================================================================
WITH cohort AS (
  SELECT e.id
  FROM public.expedientes e
  WHERE e.deleted_at IS NULL
    ...
    AND e.etapa_actual BETWEEN 3 AND 8
),
trans AS (
  SELECT
    ...
    al.created_at AS at,
    CASE
      WHEN jsonb_typeof(al.payload->'etapa_anterior') = 'number'
        THEN (al.payload->>'etapa_anterior')::int
      WHEN (al.payload->>'etapa_anterior') ~ '^[0-9]+$'
        THEN (al.payload->>'etapa_anterior')::int
      ELSE NULL
    END AS etapa_from,
    CASE
      WHEN jsonb_typeof(al.payload->'etapa_nueva') = 'number'
        THEN (al.payload->>'etapa_nueva')::int
      WHEN (al.payload->>'etapa_nueva') ~ '^[0-9]+$'
        THEN (al.payload->>'etapa_nueva')::int
      ELSE NULL
    END AS etapa_to
  FROM public.action_log al
  WHERE al.entity_type = 'expediente'
    AND al.entity_id IN (SELECT id FROM cohort)
    AND al.payload ? 'etapa_nueva'
  UNION ALL
  SELECT
    m.expediente_id,
    m.created_at,
    m.etapa_origen::int,
    m.etapa_destino::int
  FROM public.expediente_movimientos_mesa m
  WHERE m.expediente_id IN (SELECT id FROM cohort)
),
tramo_entries AS (
  SELECT *
  FROM trans
  WHERE etapa_to BETWEEN 3 AND 8
    AND (etapa_from IS NULL OR etapa_from < 3)
),
-- Episodio actual = última entrada desde <3 (cubre reentrada tras salida)
latest_entry AS (
  SELECT DISTINCT ON (expediente_id)
    expediente_id, at AS started_at
  FROM tramo_entries
  ORDER BY expediente_id, at DESC
)
UPDATE public.expedientes e
SET vigencia_documental_started_at = le.started_at
FROM latest_entry le
WHERE e.id = le.expediente_id
  AND e.vigencia_documental_started_at IS NULL;

-- =============================================================================
-- 6) Backfill liberada_at (primera llegada >=9)
-- =============================================================================
WITH targets AS (
  SELECT e.id
  FROM public.expedientes e
  WHERE e.deleted_at IS NULL
    AND (
      e.etapa_actual >= 9
      OR e.vigencia_documental_liberada_at IS NULL
    )
),
trans AS (
  SELECT
  ...
  WHERE al.entity_type = 'expediente'
    AND al.payload ? 'etapa_nueva'
  UNION ALL
  SELECT m.expediente_id, m.created_at, m.etapa_destino::int
  FROM public.expediente_movimientos_mesa m
),
first9 AS (
  SELECT expediente_id, min(at) AS liberada_at
  FROM trans
  WHERE etapa_to >= 9
  GROUP BY 1
)
UPDATE public.expedientes e
SET vigencia_documental_liberada_at = f.liberada_at
FROM first9 f
WHERE e.id = f.expediente_id
  AND e.vigencia_documental_liberada_at IS NULL
  AND (e.etapa_actual >= 9 OR TRUE);

-- =============================================================================
-- 7) Mesa override: skip assert GUC
-- =============================================================================
...
BEGIN
  ...
  -- P211: Mesa override = este RPC no llama assert_*; trigger solo clock/release.
...
-- =============================================================================
-- 8) Early assert gates on canonical forward writers (fail before P208/side-effects)
-- =============================================================================
...
BEGIN
  FOR r IN
    ...
    FOR UPDATE OF e
  LOOP
    PERFORM public.assert_expediente_vigencia_documental_ok(r.id);
    UPDATE public.expedientes
    SET
      etapa_actual = 9,
      subestado = 'en_proceso',
