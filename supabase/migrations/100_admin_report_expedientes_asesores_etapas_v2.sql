-- ConCasa CRM — P114: reporte Admin v2 con rango de fechas (Monterrey)
-- No modifica admin_report_expedientes_asesores_etapas (P112).

CREATE OR REPLACE FUNCTION public.admin_report_expedientes_asesores_etapas_v2(
...
COMMENT ON FUNCTION public.admin_report_expedientes_asesores_etapas_v2(UUID[], SMALLINT[], TEXT, DATE, DATE) IS
  'P114: reporte Super Admin v2 — asesores × paso visual + rango fechas (America/Monterrey). Históricos sin fecha excluidos solo con rango.';

REVOKE ALL ON FUNCTION public.admin_report_expedientes_asesores_etapas_v2(UUID[], SMALLINT[], TEXT, DATE, DATE) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_report_expedientes_asesores_etapas_v2(UUID[], SMALLINT[], TEXT, DATE, DATE) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_report_expedientes_asesores_etapas_v2(UUID[], SMALLINT[], TEXT, DATE, DATE) TO authenticated;
