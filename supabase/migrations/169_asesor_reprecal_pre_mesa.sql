-- ConCasa CRM — P169: reprecal / cambio de programa también PRE-MESA (extiende P168)
-- NO edita 155/168. Solo REPLACE del gate RO.
-- Criterio: un expediente activo (ciclo=activo, deleted_at IS NULL) del NSS en la org
-- se reutiliza aunque submitted_to_mesa sea false.
-- Statuses: se conservan reprecal_own_mesa / reprecal_change_programa (ahora = propio activo
-- pre o post Mesa). iniciar/resolver P168 no exigen submitted_to_mesa.
...
BEGIN
  ...
  -- Universo: activos de la org (pre o post Mesa). Sin filtro submitted_to_mesa.
  SELECT count(*) INTO v_activos_cualquier
  ...
  SELECT count(*) INTO v_activos_mismo
  FROM public.expedientes e
  WHERE e.organization_id = v_org
    AND e.nss = v_nss
  ...
  IF v_activos_cualquier > 1 OR v_activos_mismo > 1 THEN
  ...
  -- Un activo en otro programa (propio) → cambio diferido (misma semántica P168)
  IF v_activos_cualquier = 1 THEN
    SELECT e.id, e.asesor_id, e.programa, e.reprecalificacion_pendiente_id
    INTO v_exp_id, v_exp_asesor, v_exp_programa, v_pendiente
    FROM public.expedientes e
    WHERE e.organization_id = v_org
...
COMMENT ON FUNCTION public.asesor_lookup_nss_precal_gate(TEXT, public.programa) IS
  'P155/P168/P169: gate RO alta/re-precal/cambio por NSS (activo pre o post Mesa). Solo asesor. Statuses reprecal_own_mesa|reprecal_change_programa = propio activo.';
