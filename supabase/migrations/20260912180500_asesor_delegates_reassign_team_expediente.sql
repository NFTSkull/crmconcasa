-- Reasignación segura para asesores delegados del mismo equipo activo.
-- Aplica a perfiles con create_for_any_advisor + integrate_for_any_advisor.
-- No exige liderazgo ni correo específico; el alcance sigue siendo team-scoped.
...
BEGIN
  ...
  -- Actor delegado y titular actual deben compartir el mismo equipo activo.
...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR NOT public.profile_has_capability(v_actor_id, 'create_for_any_advisor')
     OR NOT public.profile_has_capability(v_actor_id, 'integrate_for_any_advisor') THEN
    RAISE EXCEPTION 'asesor_reassign_team_expediente: sin permisos delegados de creación/integración'
      USING ERRCODE = '42501';
  END IF;
  ...
  -- Mismo aislamiento que la integración delegada: actor y dueño actual
  -- deben compartir un equipo activo. No da acceso a otros equipos.
  SELECT t.* INTO v_team
  FROM public.asesor_equipos t
  WHERE t.active = true
    AND t.organization_id = v_actor.organization_id
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'asesor_reassign_team_expediente: actor y titular no comparten equipo activo'
      USING ERRCODE = '42501';
  END IF;
  ...
  -- Solo el lote de corrección todavía en borrador cambia de responsable.
  -- Lotes enviados/revisados permanecen históricos.
  ...
  -- Proyecciones de agenda activas vinculadas al expediente.
  ...
  -- Reencolar citas activas para que Drive refleje el nuevo asesor sin
  -- cambiar fecha, hora, fila ni booking.
...
COMMENT ON FUNCTION public.asesor_reassign_team_context(uuid) IS
  'Contexto de reasignación para delegados create+integrate; targets limitados al equipo activo compartido.';
COMMENT ON FUNCTION public.asesor_reassign_team_expediente(uuid, uuid) IS
  'Reasigna expediente activo dentro del equipo compartido por actor/owner; conserva relaciones y reencola agenda.';
