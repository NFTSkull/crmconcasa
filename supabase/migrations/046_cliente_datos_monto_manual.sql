-- ConCasa CRM — monto_calculado manual en save_cliente_datos
-- P046: asesor envía monto_calculado; sin recálculo automático por porcentaje
...
CREATE OR REPLACE FUNCTION public.save_cliente_datos(
  ...
  p_monto_calculado NUMERIC DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Información de cobro (monto calculado capturado por asesor)
  SELECT ed.*
  INTO v_editor
  ...
  IF p_monto_calculado IS NULL OR p_monto_calculado <= 0 THEN
    RAISE EXCEPTION 'save_cliente_datos: monto calculado es obligatorio'
      USING ERRCODE = '22023';
  END IF;

  v_monto_calculado := round(p_monto_calculado::NUMERIC, 2);
...
COMMENT ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, NUMERIC) IS
  'Asesor dueño guarda/actualiza cliente_datos. RFC opcional; cobro con monto calculado capturado por asesor.';

REVOKE ALL ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, NUMERIC) TO authenticated;
...
COMMENT ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, NUMERIC) IS
  'P3J.6: asesor dueño corrige cliente_datos rechazados por Mesa (vuelve a completo).';

REVOKE ALL ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, NUMERIC) TO authenticated;
