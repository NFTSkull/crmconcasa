-- ConCasa CRM — P136: Mesa reemplaza (vía register existente) y elimina Pagaré / Notificación / Apodaca
-- Solo allowlist Mesa + RPC eliminar. No toca etapas, citas, montos, Ingresos ni mesa.mover_etapa.

-- =============================================================================
-- A) Allowlist Mesa: + cliente_notificacion_apodaca (P136; conserva P090/P092/P096)
-- =============================================================================
...
COMMENT ON FUNCTION public.integration_doc_tipos_mesa_upload() IS
  'P030/P090/P092/P096/P136: tipos Mesa (complementarios + pagaré + notificación + solicitud + notificación Apodaca). Opcionales; no gate agenda.';

REVOKE ALL ON FUNCTION public.integration_doc_tipos_mesa_upload() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.integration_doc_tipos_mesa_upload()
  TO authenticated, service_role, postgres;

-- Tipos operativos mutables (reemplazo/eliminación P136) — subset estricto
CREATE OR REPLACE FUNCTION public.mesa_tipos_documento_operativos_mutables()
RETURNS TEXT[]
LANGUAGE sql
IMMUTABLE
SET search_path = public
...
COMMENT ON FUNCTION public.mesa_tipos_documento_operativos_mutables() IS
  'P136: tipos que Mesa puede eliminar (y reemplazar vía register_mesa_documento).';

REVOKE ALL ON FUNCTION public.mesa_tipos_documento_operativos_mutables() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.mesa_tipos_documento_operativos_mutables()
  TO authenticated, service_role, postgres;

-- =============================================================================
-- B) RPC eliminar documento activo (soft-delete)
-- =============================================================================
...
BEGIN
  ...
  -- Mismo contrato que register_mesa_documento (mesa_control_admin es alias UI de mesa_admin)
  ...
  IF NOT FOUND THEN
    -- Idempotente: sin activo, no hay efectos secundarios
  ...
  -- Sin mutar etapa, subestado, citas, montos ni ingresos.
  RETURN jsonb_build_object(
    'ok', true,
...
COMMENT ON FUNCTION public.mesa_eliminar_documento_expediente(UUID, TEXT) IS
  'P136: soft-delete del documento activo (pagaré / notificación / apodaca). Idempotente. Sin cambio de etapa.';
