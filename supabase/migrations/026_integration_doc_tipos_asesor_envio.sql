-- ConCasa CRM — P3H.1c: 8 documentos asesor para enviar_a_mesa; 10 para validación Mesa

-- =============================================================================
-- Catálogos de integración (etapa 1)
-- =============================================================================
-- Asesor envía a Mesa con 8 documentos (sin acta ni constancia SAT — los sube Mesa).
-- Mesa valida/avanza con 10 documentos (8 asesor + acta + constancia SAT).
...
COMMENT ON FUNCTION public.integration_doc_tipos_asesor_envio() IS
  'P3H.1c: tipos que el asesor debe subir antes de enviar_a_mesa (8).';

-- Lista completa integración (validación Mesa 1→2 y re-checks operativos).
...
COMMENT ON FUNCTION public.integration_doc_tipos_obligatorios() IS
  'Integración completa (10): validación Mesa y avance 1→2; incluye acta y constancia SAT.';

-- =============================================================================
-- Presentes para envío asesor (8)
-- =============================================================================
...
COMMENT ON FUNCTION public.count_integration_docs_presentes(UUID) IS
  'Cuenta documentos del asesor presentes para enviar_a_mesa (lista de 8).';

COMMENT ON FUNCTION public.integration_docs_completos(UUID) IS
  'true si los 8 documentos de integration_doc_tipos_asesor_envio están presentes.';

-- =============================================================================
-- Validados Mesa (10) — sin cambio de universo, solo comentarios
-- =============================================================================
COMMENT ON FUNCTION public.count_integration_docs_validados(UUID) IS
  'Cuenta documentos validados de integration_doc_tipos_obligatorios (lista de 10).';

COMMENT ON FUNCTION public.integration_docs_todos_validados(UUID) IS
  'true si los 10 documentos de integration_doc_tipos_obligatorios están validados.';

-- =============================================================================
-- enviar_a_mesa — mensaje de error con cardinalidad asesor (8)
-- =============================================================================
...
COMMENT ON FUNCTION public.enviar_a_mesa(UUID) IS
  'Asesor dueño envía expediente a Mesa. Gates: editor aprobado+monto, RFC, cliente_datos completo, 8 docs asesor (integration_doc_tipos_asesor_envio).';

REVOKE ALL ON FUNCTION public.integration_doc_tipos_asesor_envio() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.integration_doc_tipos_asesor_envio() TO authenticated;
