-- ConCasa CRM — campos de cobro en cliente_datos (porcentaje, monto calculado, método de pago)
-- Reversible: DROP COLUMN + restaurar RPCs previos (042).

ALTER TABLE public.cliente_datos
  ADD COLUMN IF NOT EXISTS porcentaje_cobro NUMERIC(5,2),
  ADD COLUMN IF NOT EXISTS monto_calculado NUMERIC(12,2),
  ADD COLUMN IF NOT EXISTS metodo_pago TEXT;

ALTER TABLE public.cliente_datos
  DROP CONSTRAINT IF EXISTS cliente_datos_porcentaje_cobro_chk;

ALTER TABLE public.cliente_datos
  ADD CONSTRAINT cliente_datos_porcentaje_cobro_chk
  CHECK (porcentaje_cobro IS NULL OR (porcentaje_cobro > 0 AND porcentaje_cobro <= 100));

ALTER TABLE public.cliente_datos
  DROP CONSTRAINT IF EXISTS cliente_datos_monto_calculado_chk;

ALTER TABLE public.cliente_datos
  ADD CONSTRAINT cliente_datos_monto_calculado_chk
  CHECK (monto_calculado IS NULL OR monto_calculado >= 0);

ALTER TABLE public.cliente_datos
  DROP CONSTRAINT IF EXISTS cliente_datos_metodo_pago_chk;

ALTER TABLE public.cliente_datos
  ADD CONSTRAINT cliente_datos_metodo_pago_chk
  CHECK (metodo_pago IS NULL OR btrim(metodo_pago) <> '');

DROP FUNCTION IF EXISTS public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado);
...
BEGIN
  ...
  -- Información de cobro (obligatoria; monto calculado en servidor)
  SELECT ed.*
  INTO v_editor
  ...
  IF NOT FOUND
     OR v_editor.decision IS DISTINCT FROM 'aprobado'
  ...
  v_monto_calculado := round(v_monto_aprobado * v_porcentaje / 100, 2);
...
COMMENT ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT) IS
  'Asesor dueño guarda/actualiza cliente_datos. RFC opcional; cobro obligatorio con monto calculado en servidor.';

REVOKE ALL ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT) TO authenticated;

DROP FUNCTION IF EXISTS public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB);
...
COMMENT ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT) IS
  'P3J.6: asesor dueño corrige cliente_datos rechazados por Mesa (vuelve a completo).';

REVOKE ALL ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT) TO authenticated;
...
COMMENT ON FUNCTION public.enviar_a_mesa(UUID) IS
  'Asesor dueño envía expediente a Mesa. Gates: editor aprobado+monto, cliente_datos completo+cobro, docs asesor.';
