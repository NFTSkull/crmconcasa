-- ConCasa CRM — P112: reporte Admin expedientes por asesores y pasos visuales
-- Fotografía del estado actual (sin rango de fechas). Solo super_admin. Read-only.

CREATE OR REPLACE FUNCTION public.admin_report_expedientes_asesores_etapas(
...
COMMENT ON FUNCTION public.admin_report_expedientes_asesores_etapas(UUID[], SMALLINT[], TEXT) IS
  'P112: reporte read-only Super Admin — expedientes vigentes por asesor × paso visual (11).';

REVOKE ALL ON FUNCTION public.admin_report_expedientes_asesores_etapas(UUID[], SMALLINT[], TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_report_expedientes_asesores_etapas(UUID[], SMALLINT[], TEXT) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_report_expedientes_asesores_etapas(UUID[], SMALLINT[], TEXT) TO authenticated;
