-- ConCasa CRM — P092 B1: Notificación documento (cliente_notificacion) para Mesa
-- Allowlist Mesa + MIME PDF/JPEG/PNG + gate etapa_actual >= 7 en register_mesa_documento.
-- Distinto de agenda_bookings.kind = 'notificacion' (P070 — intacto).
-- No es gate de avance. No UI. No obligatorio. Sin herencia reingreso.
-- Conserva cliente_pagare (P090) intacto.

-- =============================================================================
-- A) Allowlist Mesa: agregar cliente_notificacion (conservar pagaré)
-- =============================================================================
...
COMMENT ON FUNCTION public.integration_doc_tipos_mesa_upload() IS
  'P030/P090/P092: tipos Mesa vía register_mesa_documento (semanas, acta, SAT, pagaré, notificación doc). Opcionales; no son gate de avance. Distinto de agenda kind=notificacion.';
...
-- =============================================================================
-- B) MIME: imágenes PDF/JPEG/PNG para cliente_pagare y cliente_notificacion
-- =============================================================================
...
BEGIN
  ...
  -- Pagaré (P090) + Notificación documento (P092): PDF (arriba) + JPEG/PNG
  IF v_tipo IN ('cliente_pagare', 'cliente_notificacion')
...
COMMENT ON FUNCTION public.expediente_documento_mime_permitido(TEXT, TEXT) IS
  'MIME por tipo. PDF global. Pagaré/Notificación doc: PDF/JPEG/PNG. INE/carta/acta digital: PDF+imágenes. Acta/SAT/semanas Mesa: solo PDF.';
...
-- =============================================================================
-- C) register_mesa_documento: gate Pagaré + Notificación doc etapa >= 7
-- =============================================================================
...
COMMENT ON FUNCTION public.register_mesa_documento(UUID, TEXT, TEXT, TEXT, TEXT, BIGINT) IS
  'P030/P090/P092: Mesa registra complementarios, Pagaré y documento Notificación (etapa>=7, PDF/JPEG/PNG). Soft-delete versionado. No gate de avance. Distinto de agenda kind=notificacion.';
