-- ConCasa CRM — Notificación compartida Asesor|Mesa (`cliente_notificacion_apodaca`)
-- Quita el gate de etapa >= 7 para el tipo interno apodaca en
-- register_expediente_documento_pre_reingreso (ruta normal de register_expediente_documento).
-- Conserva el gate solo para `cliente_notificacion`.
-- Sin cambios de RLS/grants. Mesa ya permitía el tipo sin etapa.
...
COMMENT ON FUNCTION public.register_expediente_documento_pre_reingreso(uuid, text, text, text, text, bigint) IS
  'Registro/reemplazo documentos asesor (ruta normal vía register_expediente_documento). cliente_notificacion_apodaca sin gate de etapa; cliente_notificacion sigue etapa >= 7.';

REVOKE ALL ON FUNCTION public.register_expediente_documento_pre_reingreso(uuid, text, text, text, text, bigint) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.register_expediente_documento_pre_reingreso(uuid, text, text, text, text, bigint) FROM anon;
GRANT EXECUTE ON FUNCTION public.register_expediente_documento_pre_reingreso(uuid, text, text, text, text, bigint) TO authenticated;
