-- ConCasa CRM — Reingreso: corrección completa de Datos Generales + docs asesor
-- Amplía hotfix 146 (solo domicilio/estado de cuenta) a todos los tipos de
-- integration_doc_tipos_asesor_upload() mientras el reingreso esté activo.
-- Cierre: reingreso manual solo con etapa_actual = 1 (tras avanzar Mesa se cierra);
--         P072 hijo solo mientras es_reingreso_post_biometricos_valido (etapa 6).
-- No toca Acuse/Pagaré/agenda/padre/Sheets; no modifica migración 146.

-- Manual activo: mismo expediente, count > 0, enviado, ciclo activo, etapa 1.
...
COMMENT ON FUNCTION public.es_reingreso_manual_docs_editables(uuid) IS
  'True si reingreso manual activo (count>0) en etapa 1: corrección completa asesor abierta. Se cierra al avanzar Mesa.';
...
-- Alias canónico: P072 válido O reingreso manual activo (etapa 1).
CREATE OR REPLACE FUNCTION public.es_reingreso_asesor_edicion_activa(
  p_expediente_id uuid
)
RETURNS boolean
LANGUAGE sql
...
COMMENT ON FUNCTION public.es_reingreso_asesor_edicion_activa(uuid) IS
  'True si el asesor dueño puede corregir Datos Generales y todos sus docs de integración durante reingreso activo (P072 etapa 6 o manual etapa 1).';

REVOKE ALL ON FUNCTION public.es_reingreso_asesor_edicion_activa(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.es_reingreso_asesor_edicion_activa(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.es_reingreso_asesor_edicion_activa(uuid) TO authenticated;

-- Compat 146: mismo predicado (nombre histórico).
...
SET search_path TO 'public'
AS $$
  SELECT public.es_reingreso_asesor_edicion_activa(p_expediente_id);
$$;

COMMENT ON FUNCTION public.es_reingreso_docs_domicilio_estado_cuenta(uuid) IS
  'Alias de es_reingreso_asesor_edicion_activa (compat 146).';
...
-- Storage: primer upload post-Mesa de cualquier tipo asesor_upload en reingreso activo.
...
-- Wrapper: ruta reingreso para cualquier tipo de integration_doc_tipos_asesor_upload().
...
COMMENT ON FUNCTION public.register_expediente_documento(uuid, text, text, text, text, bigint) IS
  'Registro documentos asesor. Reingreso activo (P072/manual etapa 1): todos integration_doc_tipos_asesor_upload con upload/reemplazo; resto vía pre_reingreso.';
