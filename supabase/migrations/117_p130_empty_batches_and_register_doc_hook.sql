-- Fix P130: no congelar lotes vacíos + capturar reemplazos post-Mesa
-- en register_expediente_documento_pre_reingreso (vía register_expediente_documento).
-- No inventa diffs históricos; no toca filtros P129 / etapas / citas.
...
BEGIN
  ...
  -- No congelar lotes vacíos: descarta borrador sin filas (evita "Cambios · 0").
  IF NOT EXISTS (
    SELECT 1
    FROM public.expediente_asesor_cambios c
    WHERE c.lote_id = p_lote_id
  ) THEN
    DELETE FROM public.expediente_asesor_cambio_lotes l
...
COMMENT ON FUNCTION public.asesor_cambio_freeze_lote(UUID) IS
  'Congela lote a pendiente_revision solo si tiene cambios; borra borrador vacío.';
...
COMMENT ON FUNCTION public.register_expediente_documento_pre_reingreso(UUID, TEXT, TEXT, TEXT, TEXT, BIGINT) IS
  'Registro/reemplazo documental asesor (pre-reingreso). P130: post-Mesa reemplazo → asesor_cambio_record_doc_reemplazo.';

REVOKE ALL ON FUNCTION public.register_expediente_documento_pre_reingreso(
  UUID, TEXT, TEXT, TEXT, TEXT, BIGINT
) FROM PUBLIC, anon, authenticated;
-- Invocable solo vía wrapper register_expediente_documento (SECURITY DEFINER).
