-- ConCasa CRM — Hotfix inbound Sheets: ocupación manual consume cupo físico
-- Migración 162. No edita 131–161.
-- - Flags/fingerprint de ocupación manual (sin inventar booking CRM)
-- - Upsert acepta fingerprint + sheet_last_seen_at
-- - Mensaje hard-gate amigable SIN_CUPO_REAL_EN_SHEET
-- No toca capacidades, etapas, bookings existentes ni Sheets.

ALTER TABLE public.agenda_sheet_slot_inventory
  ADD COLUMN IF NOT EXISTS manual_occupancy_fingerprint TEXT NULL;

ALTER TABLE public.agenda_sheet_slot_inventory
  ADD COLUMN IF NOT EXISTS sheet_last_seen_at TIMESTAMPTZ NULL;

COMMENT ON COLUMN public.agenda_sheet_slot_inventory.manual_occupancy_fingerprint IS
  'Hash estable de NSS|NOMBRE|ASESOR cuando status=occupied_external (sin PII en logs).';
COMMENT ON COLUMN public.agenda_sheet_slot_inventory.sheet_last_seen_at IS
  'Última vez que live-sync/webhook/reconcile releyó la fila desde Google Sheets.';

-- =============================================================================
-- Upsert batch: conserva detach 141 + fingerprint / sheet_last_seen_at
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    v_fp := NULLIF(btrim(COALESCE(v_elem->>'manual_occupancy_fingerprint', '')), '');
    ...
    IF FOUND THEN
      -- Sheet vacío pero hay booking CRM activo → NO liberar (CRM_BOOKING_MISSING_FROM_SHEET)
      IF v_status = 'available' AND v_existing.booking_id IS NOT NULL THEN
        ...
        v_fp := v_existing.manual_occupancy_fingerprint;
      ELSIF v_status = 'occupied_external'
        ...
        v_occ := v_existing.occupancy_source;
        v_fp := COALESCE(v_fp, v_existing.manual_occupancy_fingerprint);
      END IF;
      ...
      SET
        ...
        manual_occupancy_fingerprint = CASE
          WHEN v_status = 'occupied_external' THEN v_fp
          WHEN v_status = 'available' THEN NULL
          ELSE COALESCE(v_fp, i.manual_occupancy_fingerprint)
        END,
        observed_at = NOW(),
    ...
    ELSE
      INSERT INTO public.agenda_sheet_slot_inventory (
        ...
        booking_id, expediente_id, occupancy_source,
        manual_occupancy_fingerprint, observed_at, sheet_last_seen_at
      ) VALUES (
        ...
        v_expediente_id,
        v_occ,
        CASE WHEN v_status = 'occupied_external' THEN v_fp ELSE NULL END,
        NOW(),
        NOW()
      );
    END IF;
...
-- =============================================================================
-- Hard gate: mensaje explícito para race Sheet manual ↔ CRM
-- (conserva código SIN_CUPO_REAL_EN_SHEET para mapeo FE)
-- =============================================================================
