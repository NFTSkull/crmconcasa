-- ConCasa CRM — Admin: snapshot stock vigente por etapa_actual (sin periodo)
-- RPCs RO separados de admin_get_mesa_cohort_by_etapa / admin_list_mesa_envios_page.
-- Auth: solo super_admin vía __admin_require_super_admin().
...
COMMENT ON FUNCTION public.admin_expedientes_snapshot_etapas(UUID, TEXT, TEXT) IS
  'Admin RO: stock vigente por etapa_actual (sin fechas). Filtros opcionales asesor/estado/buscar.';
...
COMMENT ON FUNCTION public.admin_list_expedientes_snapshot_page(INTEGER, INTEGER, UUID, SMALLINT, TEXT, TEXT) IS
  'Admin RO: listado paginado del stock vigente (sin fechas). Alineado con admin_expedientes_snapshot_etapas.';
