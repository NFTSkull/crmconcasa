-- ConCasa CRM — Dashboard Silvia: distribución por etapa solo de enviados a Mesa.
-- Read-model only. No muta expedientes, etapas, ciclos ni fechas.
-- Mantiene KPIs total/activos/cerrados de todo el equipo; solo `by_etapa`
-- se limita a submitted_to_mesa=true cuando el líder autenticado es Silvia Reyes.
...
COMMENT ON FUNCTION public.asesor_lider_get_dashboard(uuid, date, date) IS
  'KPIs equipo líder. Para Silvia Reyes, by_etapa muestra solo expedientes enviados a Mesa; total/activos/cerrados conservan el universo completo del equipo.';
