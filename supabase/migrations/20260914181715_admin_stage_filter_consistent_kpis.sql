-- ConCasa CRM — Admin: etapa seleccionada gobierna KPIs, producción y precalificaciones.
-- READ-MODEL ONLY. Nuevas RPC v2; no UPDATE/backfill/writers.
...
COMMENT ON FUNCTION public.admin_get_production_summary_v2(timestamptz,timestamptz,uuid,smallint[],text) IS
  'Admin read model v2: periodo + asesor + estado + etapas actuales aplican al mismo universo de todos los KPIs.';
...
COMMENT ON FUNCTION public.admin_list_production_by_asesor_v2(timestamptz,timestamptz,text,uuid,smallint[]) IS
  'Admin read model v2: métricas por asesor recalculadas sobre las etapas actuales seleccionadas.';
...
COMMENT ON FUNCTION public.admin_list_precalificaciones_page_v2(timestamptz,timestamptz,integer,integer,uuid,text,text,smallint[],text) IS
  'Admin read model v2: tabla y resumen de precalificaciones respetan etapa actual y estado global.';