-- ConCasa CRM — asesor puede registrar monto_aprobado sin cambiar decision del editor
-- P045: RPC asesor_update_monto_aprobado + gate por monto > 0 (no decision aprobado)
...
BEGIN
  ...
  IF v_exp.asesor_id IS DISTINCT FROM v_actor_id THEN
    RAISE EXCEPTION 'asesor_update_monto_aprobado: solo el asesor dueño puede actualizar el monto'
      USING ERRCODE = '42501';
  END IF;
...
COMMENT ON FUNCTION public.asesor_update_monto_aprobado(UUID, NUMERIC) IS
  'Asesor dueño registra monto_aprobado sin modificar decision del editor.';
...
COMMENT ON FUNCTION public.enviar_a_mesa(UUID) IS
  'Asesor dueño envía expediente a Mesa. Gates: monto_aprobado>0, cliente_datos completo+cobro, docs asesor.';
