-- ConCasa CRM — P199: Disponibles = trabajo accionable libre de Mesa.
-- Cloud max = 198. Citas → 200.
-- CREATE OR REPLACE read-model. 0 tablas. 0 columnas. 0 backfill. 0 take (Fase 0: 0 pending libres en en_espera_asesor).

CREATE OR REPLACE FUNCTION public.mesa_es_trabajo_accionable_mesa(
  p_ciclo TEXT,
  p_subestado TEXT,
  p_categoria TEXT,
  p_cambio_estado TEXT
)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
...
AS $$
  SELECT
    coalesce(p_ciclo, '') = 'activo'
    AND (
      coalesce(p_cambio_estado, '') IN (
        'CORRECTION_PENDING_REVIEW',
        'ADVISOR_UPDATE_PENDING_REVIEW'
      )
      OR (
        p_subestado IS DISTINCT FROM 'rechazado'
        AND p_categoria IS DISTINCT FROM 'correccion_requerida'
        AND p_cambio_estado IS DISTINCT FROM 'WAITING_ADVISOR'
      )
    );
$$;

COMMENT ON FUNCTION public.mesa_es_trabajo_accionable_mesa(TEXT, TEXT, TEXT, TEXT) IS
  'P199: hay trabajo Mesa ahora. Asignación (assigned_to/estado_mesa) la aplica mesa_list_bandeja_page.';

REVOKE ALL ON FUNCTION public.mesa_es_trabajo_accionable_mesa(TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.mesa_es_trabajo_accionable_mesa(TEXT, TEXT, TEXT, TEXT) FROM anon;
GRANT EXECUTE ON FUNCTION public.mesa_es_trabajo_accionable_mesa(TEXT, TEXT, TEXT, TEXT) TO authenticated;
    ...
    WHERE
      ...
      AND CASE v_ops
        ...
        WHEN 'sin_asignar' THEN
          -- P195: cl.subestado IS DISTINCT FROM 'rechazado'
          --       cl.categoria IS DISTINCT FROM 'correccion_requerida'
          --       cl.assigned_to IS NULL
          cl.ciclo_estado = 'activo'
          AND cl.assigned_to IS NULL
          AND (cl.estado_mesa IS NULL OR cl.estado_mesa = 'sin_asignar')
          AND public.mesa_es_trabajo_accionable_mesa(
            cl.ciclo_estado,
            cl.subestado,
            cl.categoria,
            cl.cambio_eff->>'estado'
          )
...
COMMENT ON FUNCTION public.mesa_list_bandeja_page(
  ...
  'P199: Disponibles = trabajo Mesa accionable y libre. P198 intacto. Sin writers.';
