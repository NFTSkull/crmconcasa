-- ConCasa CRM — P206: Disponibles = TODO trabajo accionable Mesa (P199).
-- Cloud max = 205. Citas ops / counts P205 intactos.
-- Solo cambia predicado p_ops_filter=sin_asignar: deja de exigir assigned_to NULL / estado_mesa libre.
-- 0 writers. 0 backfill. 0 tablas. mesa_es_trabajo_accionable_mesa / take / release intactos.
    ...
    WHERE
      ...
      AND CASE v_ops
        ...
        WHEN 'sin_asignar' THEN
          -- P206: Disponibles = TODO trabajo accionable P199 (assignment no oculta).
          -- assigned_to / estado_mesa siguen en payload para badges; no filtran membresía.
          cl.ciclo_estado = 'activo'
...
COMMENT ON FUNCTION public.mesa_list_bandeja_page(
  ...
  'P206: ops sin_asignar (Disponibles) = todo trabajo accionable P199; assignment no filtra. P198/counts/take/release intactos.';
