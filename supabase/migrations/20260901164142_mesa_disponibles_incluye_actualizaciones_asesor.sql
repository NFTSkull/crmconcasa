-- ConCasa CRM — P207.1: Disponibles = Nuevos ∪ CORRECTION_PENDING ∪ ADVISOR_UPDATE pending.
-- Cloud max revisar al apply. 0 writers. P198/P199/take/release intactos.
    ...
    WHERE
      ...
      AND CASE v_ops
        ...
        WHEN 'sin_asignar' THEN
          -- P207.1: todo cambio pendiente de revisión ∪ Nuevos plain. Assignment no oculta.
          cl.ciclo_estado = 'activo'
          AND (
            cl.cambio_eff->>'estado' IN (
...
COMMENT ON FUNCTION public.mesa_list_bandeja_page(
  ...
  'P207.1: ops sin_asignar (Disponibles) = Nuevos ∪ CORRECTION_PENDING ∪ ADVISOR_UPDATE pending; assignment no filtra.';
...
-- P207.1-B: counts_fast — campo `disponibles` alineado con ops sin_asignar (paridad list/chip).
...
COMMENT ON FUNCTION public.mesa_bandeja_counts_fast(TEXT, TEXT) IS
  'P207.1: KPIs Mesa. Campo disponibles alineado con ops sin_asignar (P207.1 membresía).';
