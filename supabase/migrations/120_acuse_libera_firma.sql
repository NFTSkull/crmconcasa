-- ConCasa CRM — P132-acuse: Acuse libera firma (8→9 + firma_agendable_desde;
-- Bio Mesa 5→8; Notificación deja de avanzar 7→9).
-- Sin backfill. No modifica migraciones 001–119. No toca Super Admin / P130 / citas notif.

-- =============================================================================
-- Stub no-op: expediente_apply_notificacion_7_9 (deja de avanzar)
-- =============================================================================
...
AS $$
BEGIN
  -- P132-acuse: Notificación ya no dispara 7→9 ni firma_agendable_desde.
...
COMMENT ON FUNCTION public.expediente_apply_notificacion_7_9(UUID, UUID, public.app_role, TEXT, UUID) IS
  'P132-acuse: stub no-op. Gate de firma pasó a Acuse (register_expediente_documento_retencion).';
...
COMMENT ON COLUMN public.expedientes.firma_agendable_desde IS
  'P132-acuse: primera fecha local (America/Monterrey) seleccionable para agendar firma (5º día hábil tras Acuse principal en etapa 8). NULL = histórico sin gate.';


-- =============================================================================
-- register_expediente_documento_retencion: restaura avance 8→9 + firma_agendable_desde
-- =============================================================================
...
BEGIN
  ...
  -- + firma_agendable_desde solo si NULL (5 días hábiles Monterrey).
...
COMMENT ON FUNCTION public.register_expediente_documento_retencion(uuid, text, text, text, text, bigint) IS
  'P132-acuse/P117: registra retención; principal en etapa 8 avanza 8→9 y fija firma_agendable_desde si NULL (5 hábiles Monterrey).';
...
-- =============================================================================
-- register_mesa_documento: sin invoke expediente_apply_notificacion_7_9
-- =============================================================================
...
COMMENT ON FUNCTION public.register_mesa_documento(uuid, text, text, text, text, bigint) IS
  'Mesa: registra documento (incl. Notificación). P132-acuse: sin avance 7→9.';


-- =============================================================================
-- register_expediente_documento_pre_reingreso: sin invoke apply 7_9
-- =============================================================================
...
-- =============================================================================
-- avanzar_etapa_operativa_pre_reingreso: rama 5 → 5→8 (transition 5_8)
-- =============================================================================
...
COMMENT ON FUNCTION public.avanzar_etapa_operativa_pre_reingreso(uuid, text) IS
  'Mesa: avances operativos gated. P132-acuse: cierre Biometría 5→8 (Acuse); conserva 6→7, 7→8, 8→9, 10→11, 11→12.';
