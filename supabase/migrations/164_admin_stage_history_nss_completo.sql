-- ConCasa CRM — P164: NSS completo en reporte histórico de etapas (Admin)
-- Solo super_admin (gate vigente). Sin máscara ****. Sin backfill. Sin tocar P163 calibración de modos.
-- Paridad con P154 (cohorte): nss_completo = true en page.
...
COMMENT ON FUNCTION public.admin_stage_history_report_page(INTEGER, INTEGER, UUID[], SMALLINT[], TEXT, DATE, DATE, TEXT, TEXT) IS
  'P164 Admin RO: detalle paginado calibrado con NSS completo (solo super_admin; sin máscara).';
...
GRANT EXECUTE ON FUNCTION public.admin_stage_history_report_page(INTEGER, INTEGER, UUID[], SMALLINT[], TEXT, DATE, DATE, TEXT, TEXT)
  TO authenticated;
