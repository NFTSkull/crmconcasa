-- ConCasa CRM — P050: asesor display batch + dirección obligatoria en save_cliente_datos
-- Sin tocar NSS/049, INE, cobro +3000, biométricos, firmas, agenda ni RLS general.

CREATE OR REPLACE FUNCTION public.get_asesor_display_batch(p_asesor_ids UUID[])
...
COMMENT ON FUNCTION public.get_asesor_display_batch(UUID[]) IS
  'Devuelve nombre/email de asesores visibles para el actor (Mesa/admin sin policy profiles).';

REVOKE ALL ON FUNCTION public.get_asesor_display_batch(UUID[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_asesor_display_batch(UUID[]) TO authenticated;
...
BEGIN
  ...
  v_direccion := btrim(COALESCE(p_direccion_opcional, ''));
  IF v_direccion = '' THEN
    RAISE EXCEPTION 'save_cliente_datos: La dirección es obligatoria.'
      USING ERRCODE = '22023';
  END IF;

  UPDATE public.expedientes
  SET direccion_opcional = v_direccion,
      updated_at = NOW()
  ...
  v_monto_calculado := round(v_monto_aprobado * v_porcentaje / 100 + 3000, 2);
...
COMMENT ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, TEXT) IS
  'Asesor dueño guarda/actualiza cliente_datos. RFC opcional; dirección obligatoria; cobro automático (+$3,000).';

REVOKE ALL ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, public.cliente_datos_estado, NUMERIC, TEXT, TEXT) TO authenticated;
...
COMMENT ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, TEXT) IS
  'P3J.6: asesor dueño corrige cliente_datos rechazados por Mesa (vuelve a completo).';

REVOKE ALL ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_cliente_datos_correccion(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, NUMERIC, TEXT, TEXT) TO authenticated;
