-- ConCasa CRM — P207.3: colas de cambios excluyen Firmados (etapa≥11).
-- Mig 20260901194500 (post-P208). NOT_FIRMADO_CAMBIOS: etapa_actual < 11 ∧ pago_concasa_resultado IS NULL.
-- P207.2 NOT_TERMINAL_PAGO se conserva en en_proceso/mi_bandeja/en_trabajo/citasHoy→<11.
-- todo_mesa/totalBandeja/rechazos intactos. 0 writers.
    ...
    WHERE
      ...
      AND CASE v_ops
        ...
        WHEN 'sin_asignar' THEN
          -- P207.3: P207.1 ∧ NOT_FIRMADO_CAMBIOS en pending; Nuevos plain intactos.
          cl.ciclo_estado = 'activo'
          AND cl.pago_concasa_resultado IS NULL
          AND (
            (
...
COMMENT ON FUNCTION public.mesa_list_bandeja_page(
  ...
  'P207.3: colas cambios excluyen Firmados (etapa≥11); en_proceso/mi_bandeja/en_trabajo conservan etapa11; todo_mesa histórico.';
...
-- P207.3-B: counts_fast — alineado con ops sin_asignar + gates Firmados en cambios.
...
COMMENT ON FUNCTION public.mesa_bandeja_counts_fast(TEXT, TEXT) IS
  'P207.3: KPIs cambios excluyen Firmados; enProceso/mi_bandeja intactos; totalBandeja/rechazos intactos.';
