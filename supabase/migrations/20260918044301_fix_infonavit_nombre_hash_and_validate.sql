-- ConCasa CRM — hotfix P218 nombre Infonavit:
-- upstream representa Ñ como # en algunos nombres (PI#A, MU#OZ, etc.).
-- Normaliza ese artefacto y rechaza cualquier otro símbolo inesperado antes de persistir.
-- Sin backfill: la reparación histórica se ejecuta aparte con precondiciones estrictas.
...
BEGIN
  ...
  IF v_nombre_raw IS NULL OR p_expediente_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'nombre_o_expediente_vacio');
  END IF;

  -- Infonavit/scraper upstream usa # como sustituto de Ñ.
  v_nombre := replace(v_nombre_raw, '#', 'Ñ');
  v_hash_normalizado := v_nombre IS DISTINCT FROM v_nombre_raw;

  -- Defensa final: jamás persistir un nombre que el propio formulario no puede guardar.
  ...
  IF NOT public.profile_has_capability(
    v_exp.asesor_id,
    'autofill_nombre_infonavit'
  ) THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'sin_capability');
  END IF;

  -- Convención de la pantalla Infonavit: ApellidoPaterno ApellidoMaterno Nombre(s)
...
REVOKE EXECUTE ON FUNCTION public.auto_fill_nombre_infonavit(uuid, text)
