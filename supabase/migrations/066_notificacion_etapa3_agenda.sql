-- ConCasa CRM — P066: Notificación etapa 3 (rama extraordinaria)
-- kind='notificacion': fecha + 12:00 fijo, sin cupo, Mesa 3→5; cancel/reagendar en etapa 3.
-- Biométricos normal: book desde etapa 3 avanza a etapa 4; Mesa 4→5 intacto.
-- Requiere migración 065_booking_kind_notificacion.

CREATE UNIQUE INDEX IF NOT EXISTS agenda_bookings_one_active_notificacion_per_expediente_idx
  ON public.agenda_bookings (expediente_id, kind)
  WHERE kind = 'notificacion'::public.booking_kind
    AND status = 'booked'::public.booking_status;


CREATE OR REPLACE FUNCTION public.book_notificacion_etapa3(
...
COMMENT ON FUNCTION public.book_notificacion_etapa3(UUID, DATE, TEXT) IS
  'Asesor dueño agenda notificación etapa 3 (12:00 fijo, sin cupo). No avanza etapa; Mesa aprueba 3→5.';

REVOKE ALL ON FUNCTION public.book_notificacion_etapa3(UUID, DATE, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.book_notificacion_etapa3(UUID, DATE, TEXT) TO authenticated;


CREATE OR REPLACE FUNCTION public.cancel_notificacion_etapa3(
  p_expediente_id UUID,
...
BEGIN
  v_actor_id := public.current_profile_id();
  IF v_actor_id IS NULL THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: usuario no autenticado' USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: perfil no encontrado o inactivo' USING ERRCODE = '42501';
  END IF;

  IF v_actor_role NOT IN ('asesor', 'mesa_admin', 'super_admin') THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: rol no autorizado (%)', v_actor_role USING ERRCODE = '42501';
  END IF;

  IF p_expediente_id IS NULL THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: expediente_id es obligatorio' USING ERRCODE = '22023';
  END IF;

  v_motivo := NULLIF(btrim(COALESCE(p_motivo, '')), '');

  IF v_actor_role <> 'asesor' AND v_motivo IS NULL THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: el motivo es obligatorio para Mesa' USING ERRCODE = '22023';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: expediente no encontrado' USING ERRCODE = 'P0002';
  END IF;

  IF v_exp.deleted_at IS NOT NULL THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: expediente no disponible' USING ERRCODE = 'P0002';
  END IF;

  IF v_actor_role <> 'super_admin'
     AND v_exp.organization_id IS DISTINCT FROM v_org_id THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: expediente fuera de la organización del actor' USING ERRCODE = '42501';
  END IF;

  IF v_actor_role = 'asesor' AND v_exp.asesor_id IS DISTINCT FROM v_actor_id THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: solo el asesor dueño puede cancelar notificación' USING ERRCODE = '42501';
  END IF;

  IF v_actor_role IN ('mesa_admin', 'super_admin')
     AND NOT public.can_see_expediente(p_expediente_id) THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: no autorizado para operar este expediente' USING ERRCODE = '42501';
  END IF;

  IF v_exp.ciclo_estado <> 'activo' THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: el expediente no está en ciclo activo' USING ERRCODE = '22023';
  END IF;

  IF v_exp.submitted_to_mesa IS NOT TRUE THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: el expediente no ha sido enviado a Mesa' USING ERRCODE = '22023';
  END IF;

  IF v_exp.subestado <> 'en_proceso' THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: subestado debe ser en_proceso (actual: %)', v_exp.subestado USING ERRCODE = '22023';
  END IF;

  IF v_exp.etapa_actual <> 3 THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: solo se puede cancelar en etapa 3 (actual: %)', v_exp.etapa_actual USING ERRCODE = '22023';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'cancel_notificacion_etapa3: no hay notificación activa para cancelar' USING ERRCODE = '22023';
  END IF;
  ...
  PERFORM public.log_action(
    v_exp.organization_id, v_actor_id, v_actor_role,
    'agenda.notificacion.cancel', 'agenda_booking', v_booking.id,
    jsonb_build_object(
      'actor_id', v_actor_id,
...
COMMENT ON FUNCTION public.cancel_notificacion_etapa3(UUID, TEXT) IS
  'Cancela notificación activa en etapa 3. Asesor dueño o mesa_admin/super_admin. Conserva historial cancelled; no cambia etapa.';

REVOKE ALL ON FUNCTION public.cancel_notificacion_etapa3(UUID, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.cancel_notificacion_etapa3(UUID, TEXT) TO authenticated;


CREATE OR REPLACE FUNCTION public.reagendar_notificacion_etapa3(
  p_expediente_id UUID,
  p_booking_date DATE,
  p_note TEXT DEFAULT NULL
)
...
COMMENT ON FUNCTION public.reagendar_notificacion_etapa3(UUID, DATE, TEXT) IS
  'Asesor dueño reagenda notificación en etapa 3: cancela anterior + nueva a las 12:00, sin cupo.';

REVOKE ALL ON FUNCTION public.reagendar_notificacion_etapa3(UUID, DATE, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reagendar_notificacion_etapa3(UUID, DATE, TEXT) TO authenticated;
