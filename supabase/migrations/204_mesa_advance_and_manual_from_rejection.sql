-- ConCasa CRM — P204-D: avance normal + movimiento manual desde rechazo operativo
-- Orquestación atómica: reactivar (si abierto) + avance/mover en la misma transacción.
-- 0 backfill. No redefine el comportamiento de callers no-Mesa de avanzar_etapa_operativa
-- vía wrapper Mesa dedicado; mesa_mover sí acepta rechazado con reactivación canónica.

CREATE OR REPLACE FUNCTION public.mesa_avanzar_etapa_reactivando_si_necesario(
...
BEGIN
  ...
  IF v_actor_id IS NULL THEN
    RAISE EXCEPTION 'MESA_ADVANCE_UNAUTHORIZED: usuario no autenticado'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT FOUND OR v_actor_role NOT IN (
    ...
    RAISE EXCEPTION 'MESA_ADVANCE_UNAUTHORIZED: perfil inactivo o rol no autorizado'
      USING ERRCODE = '42501';
  END IF;

  IF p_expediente_id IS NULL THEN
    RAISE EXCEPTION 'MESA_ADVANCE_NOT_FOUND: expediente_id es obligatorio'
      USING ERRCODE = 'P0002';
  END IF;
  ...
  IF NOT FOUND OR v_exp.deleted_at IS NOT NULL THEN
    RAISE EXCEPTION 'MESA_ADVANCE_NOT_FOUND: expediente no disponible'
      USING ERRCODE = 'P0002';
  END IF;

  IF v_actor_role <> 'super_admin'
     AND v_exp.organization_id IS DISTINCT FROM v_actor_org THEN
    RAISE EXCEPTION 'MESA_ADVANCE_UNAUTHORIZED: expediente fuera de la organización'
      USING ERRCODE = '42501';
  END IF;

  IF NOT public.can_see_expediente(p_expediente_id) THEN
    RAISE EXCEPTION 'MESA_ADVANCE_UNAUTHORIZED: expediente no visible'
      USING ERRCODE = '42501';
  END IF;

  IF v_exp.submitted_to_mesa IS NOT TRUE THEN
    RAISE EXCEPTION 'MESA_ADVANCE_NOT_SUBMITTED: expediente no enviado a Mesa'
      USING ERRCODE = '22023';
  END IF;

  IF v_exp.ciclo_estado <> 'activo' THEN
    RAISE EXCEPTION 'MESA_ADVANCE_CYCLE_NOT_ACTIVE: ciclo no activo'
      USING ERRCODE = '22023';
  END IF;

  -- Reactivación canónica solo si el rechazo operativo sigue abierto.
  IF v_exp.subestado = 'rechazado' THEN
    v_reactivacion := public.reactivar_expediente_rechazado(p_expediente_id);
  END IF;

  -- Gates normales intactos (delegación canónica).
  v_avance := public.avanzar_etapa_operativa(p_expediente_id, p_comentario);

  IF v_avance IS NULL OR jsonb_typeof(v_avance) <> 'object' THEN
    RAISE EXCEPTION 'MESA_ADVANCE_EMPTY: respuesta vacía de avance'
      USING ERRCODE = 'XX000';
  END IF;

  RETURN v_avance || jsonb_build_object(
    ...
    'rechazo_id', CASE
      WHEN v_reactivacion IS NULL THEN NULL
      ELSE v_reactivacion ->> 'rechazo_id'
    END
  );
END;
$$;

COMMENT ON FUNCTION public.mesa_avanzar_etapa_reactivando_si_necesario(UUID, TEXT) IS
  'P204-D Mesa: si hay rechazo operativo abierto, reactiva canónicamente y luego ejecuta avanzar_etapa_operativa en la misma transacción (rollback atómico si el avance falla).';

REVOKE ALL ON FUNCTION public.mesa_avanzar_etapa_reactivando_si_necesario(UUID, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.mesa_avanzar_etapa_reactivando_si_necesario(UUID, TEXT) FROM anon;
GRANT EXECUTE ON FUNCTION public.mesa_avanzar_etapa_reactivando_si_necesario(UUID, TEXT)
  TO authenticated, service_role, postgres;

-- Movimiento manual: override real. Si subestado=rechazado con rechazo abierto,
-- reactiva canónicamente y luego mueve (misma transacción).
...
COMMENT ON FUNCTION public.mesa_mover_etapa_operativa(UUID, SMALLINT, SMALLINT, TEXT) IS
  'P204-D/P076: movimiento manual Mesa entre etapas 1–12. Acepta pendiente/en_validacion_mesa/en_proceso; si rechazado, reactiva canónicamente y mueve en la misma transacción. No aplica gates de flujo normal.';
