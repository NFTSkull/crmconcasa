...
- **Resultados operativos Bernardo (mig. 165):** tabla `agenda_sheet_operational_results` (proyección reporting, **distinta** del inventario de cupo). Clasifica E/F/I del Sheet (`BIOMETRICOS` / `NOTIFICACION` / `COMPLETO✔`). Webhook upserta proyección incluso con booking en P; Apps Script notifica ediciones E–I; reconcile backfill por pestaña. RPCs `bernardo_ops_summary` / `bernardo_ops_detail` (solo `super_admin`). Periodo = `booking_date` del tab (no `last_seen_at`). KPI cuenta solo `COMPLETED`.
