...
- **P207.1 (LOCAL):** REPLACE `mesa_list_bandeja_page` rama `sin_asignar` + campo `disponibles` en `mesa_bandeja_counts_fast`. Disponibles = ciclo activo ∧ (`CORRECTION_PENDING_REVIEW` ∨ `ADVISOR_UPDATE_PENDING_REVIEW` ∨ (Nuevos plain ∧ no `WAITING_ADVISOR` ∧ categoria ≠ `correccion_requerida`)). Assignment no oculta. Paridad list/chip (`total_count` = `counts_fast.disponibles`). Quick filters P198 intactos. 0 writers. Mig `20260901164142`. Sin Cloud aún.
