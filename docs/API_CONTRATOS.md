...
**P189 mig 190 (parser dirección, LOCAL, no Cloud):** `CREATE OR REPLACE` de `infonavit_parse_direccion_mx` + `vivienda.lote`/`manzana` desde el parser. Firma `(p_raw text) → jsonb` intacta. Numeración: **189** = mapping v2; **190** = parser parity; Operación de citas local vieja 189 → **191** al retomar. 0 regeneración de históricos en este bloque.
