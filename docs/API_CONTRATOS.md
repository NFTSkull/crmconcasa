...
- Max **1** por run (`AUTO_PRECAL_RETRY_LIMIT`), **secuencial** (`await` en for; nunca `Promise.all`).  
...
- Candidatos: `expediente_precalificacion_intentos.decision='pendiente'` **y** ≥1 fila `auto_reprecal_intentos` con `resultado='pending_error'` + razón reintentable.  
- Cooldown ≥5 min, sin tope de intentos, max **2** secuencial (`AUTO_REPRECAL_RETRY_LIMIT`).  
- Schedule: `vercel.json` `* * * * *` (mismo minuto que altas; Vercel Cron no permite desfase en segundos).
...
- **Perfil captura paquete externos (FE Parte B):** dueño con `asesor_es_paquete_documental_externos(asesor_id)` → `perfilCaptura=asesor_equipo_silvia_simplificado` (nombre histórico; SQL = Silvia u Orlando). Vista UI simplificada solo si el **actor JWT** confirma paquete externos (fail-closed → completo). Docs obligatorios vía `asesor_documentos_obligatorios_envio(ownerId)` (4 internos / 8 externos). Mesa FE alineada al mismo RPC por dueño. **Teléfono de casa:** además del perfil, `origen_mesa='externo'` desactiva el campo aunque el formulario sea completo.
...
**P220 (LOCAL + Cloud):** REPLACE `integration_doc_tipos_asesor_upload_para` — externos = `envio_para`(8) + `cliente_acta_nacimiento_digital` + `cliente_constancia_situacion_fiscal` (opcionales). `envio_para` intacto (8). Sin `cliente_constancia_sat` (Mesa). Internos = `upload()` global. UI: `shouldMountAsesorConstanciaSituacionFiscalForActor` (externo+interno; unresolved fail-safe). Evidencia sigue solo internos.

**P224:** REPLACE `upload_para` — añade opcionales `cliente_semanas_cotizadas` + `cliente_vigencia_derechos` (upload=12). `envio_para` sigue en 8. FE: checklist externos = Acta+Semanas; Vigencia vía `shouldMountAsesorVigenciaDerechosForActor` (interno+externo); Evidencia sigue solo internos. Sin backfill.
...
- **P207 (LOCAL):** REPLACE `mesa_list_bandeja_page` rama `sin_asignar`. Disponibles = ciclo activo ∧ (`CORRECTION_PENDING_REVIEW` ∨ (Nuevos en Mesa ∧ no `WAITING_ADVISOR` ∧ categoria ≠ `correccion_requerida`)). **Sin** `assigned_to` / `estado_mesa`. **No** usa `mesa_es_trabajo_accionable_mesa`. Quick filters / P205 counts / P198 / P199 / take/release intactos. 0 writers. Mig **207**. **Superado en membresía de Disponibles por P207.1** (incluye `ADVISOR_UPDATE_PENDING_REVIEW`).

- **P207.3 (LOCAL):** REPLACE `mesa_list_bandeja_page` + `mesa_bandeja_counts_fast`. Gate **NOT_FIRMADO_CAMBIOS** (`etapa_actual < 11` ∧ NOT_TERMINAL_PAGO) en quick cambios (`correccion_enviada`, `correccion_solicitada`, `otras_actualizaciones`), ops `sin_asignar` (rama pending), `en_espera_asesor` y KPIs operativos de cambios (`disponibles` pending, `correccionesEnviadas`, `correccionesSolicitadas`, `otrasActualizaciones`, `enEsperaAsesor`, `citasHoy`). **No** en `en_proceso`, `mi_bandeja`, `en_trabajo`, `todo_mesa`, `totalBandeja`, rechazos. FE bandeja: `cambioBatchId` primario + hidratación enrich. Mig `20260901194500` (post-P208). Sin Cloud aún.

- **P207.2 (LOCAL):** REPLACE `mesa_list_bandeja_page` + `mesa_bandeja_counts_fast`. Gate **NOT_TERMINAL_PAGO** (`etapa_actual < 12` ∧ `pago_concasa_resultado IS NULL`) en colas operativas: quick (`correccion_enviada`, `correccion_solicitada`, `otras_actualizaciones`, `nuevos`, `en_proceso`), ops (`sin_asignar`, `en_espera_asesor`, `mi_bandeja`, `en_trabajo`) y KPIs operativos (`disponibles`, `correccionesEnviadas`, `correccionesSolicitadas`, `otrasActualizaciones`, `nuevos`, `enProceso`, `citasHoy`, `enValidacionMesa`, `enEsperaAsesor`). `todo_mesa` / `totalBandeja` / rechazos intactos. 0 writers. Mig `20260901171420`. Sin Cloud aún.

- **P207.1 (CLOUD):** REPLACE `mesa_list_bandeja_page` rama `sin_asignar` + campo `disponibles` en counts. Disponibles = ciclo activo ∧ (`CORRECTION_PENDING_REVIEW` ∨ `ADVISOR_UPDATE_PENDING_REVIEW` ∨ Nuevos plain). Assignment no oculta. Mig `20260901164142`. **Superado en gate Disponibles por P207.2** (excluye Pago ConCasa finalizado).
...
### RPCs (SECURITY DEFINER, JWT asesor activo)

| RPC | Auth / gates | Respuesta (jsonb) |
|---|---|---|
