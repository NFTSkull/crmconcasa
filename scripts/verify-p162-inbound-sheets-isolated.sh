#!/usr/bin/env bash
# Runner aislado B1.5 P162: mig 161 + rpc_agenda_sheet_manual_occupancy_p162.sql
# Skip 061/078 (Auth UID Cloud ausente en local). NO aplica Cloud. NO toca UI.
set -euo pipefail
...
ISOLATED_DB="${ISOLATED_DB_NAME:-crm_p162_iso_$$}"
TMP_DIR="$(mktemp -d /tmp/crm-p162-XXXXXX)"
...
echo "==> Isolated P162 DB: ${ISOLATED_DB}"
...
echo "==> Running supabase/tests/rpc_agenda_sheet_manual_occupancy_p162.sql"
psql_iso -f "supabase/tests/rpc_agenda_sheet_manual_occupancy_p162.sql"
echo "P162 inbound sheets SQL: ALL PASSED"
