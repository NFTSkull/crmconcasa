#!/usr/bin/env bash
# Runner aislado hotfix 192: predicado P130 → correccion_enviada Mesa/Asesor.
# Skip 061/078 (Auth UID piloto). 0 Cloud writes.
set -euo pipefail
...
ISOLATED_DB="${ISOLATED_DB_NAME:-crm_p192_iso_$$}"
TMP_DIR="$(mktemp -d /tmp/crm-p192-XXXXXX)"
...
echo "==> Isolated P192 DB: ${ISOLATED_DB}"
...
run_sql "supabase/tests/rpc_correcciones_enviadas_p130_mesa_p192.sql"
run_sql "supabase/tests/rpc_mesa_list_bandeja_page.sql"
run_sql "supabase/tests/rpc_asesor_cambios_p130.sql"
run_sql "supabase/tests/rpc_asesor_cambios_p130_empty_batch.sql"
run_sql "supabase/tests/rpc_asesor_inbox_page_p161.sql"
run_sql "supabase/tests/rpc_asesor_pendientes_p167.sql"
run_sql "supabase/tests/rpc_asesor_tareas_terminales_p191.sql"

echo "==> EXPLAIN helper (índice exp_status)"
psql_iso -c "EXPLAIN (FORMAT TEXT) SELECT public.expediente_tiene_correccion_asesor_pendiente('00000000-0000-4000-8000-000000000001'::uuid);"

echo "P192 correcciones P130 SQL: ALL PASSED"
