#!/usr/bin/env bash
# Runner aislado P167: mig 167 + rpc_asesor_pendientes_p167.sql
# Skip 061/078. NO aplica Cloud. NO toca UI.
set -euo pipefail
...
ISOLATED_DB="${ISOLATED_DB_NAME:-crm_p167_iso_$$}"
TMP_DIR="$(mktemp -d /tmp/crm-p167-XXXXXX)"
...
echo "==> Isolated P167 DB: ${ISOLATED_DB}"
...
echo "==> Running supabase/tests/rpc_asesor_pendientes_p167.sql"
if ! psql_iso -f "supabase/tests/rpc_asesor_pendientes_p167.sql"; then
  echo "ERROR: P167 SQL tests FAILED"
  exit 1
fi
echo "P167 asesor pendientes SQL: ALL PASSED"
