#!/usr/bin/env bash
# Runner aislado P165: mig 165 + rpc_bernardo_ops_p165.sql
# Skip 061/078 (Auth UID Cloud ausente en local). NO aplica Cloud.
set -euo pipefail
...
ISOLATED_DB="${ISOLATED_DB_NAME:-crm_p165_iso_$$}"
TMP_DIR="$(mktemp -d /tmp/crm-p165-XXXXXX)"
...
echo "==> Isolated P165 DB: ${ISOLATED_DB}"
...
echo "==> Running supabase/tests/rpc_bernardo_ops_p165.sql"
psql_iso -f "supabase/tests/rpc_bernardo_ops_p165.sql"

echo "==> P165 isolated SQL PASS"
