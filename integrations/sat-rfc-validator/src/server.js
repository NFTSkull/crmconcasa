...
app.listen(PORT, '0.0.0.0', () => {
  const mode = MODE === 'live' ? 'live' : 'fixture'
  const hasCapsolverKey = Boolean(String(process.env.CAPSOLVER_API_KEY || '').trim())
  const hasSecret = Boolean(String(SECRET || '').trim())
  console.log(
    `[sat-validator] listening port=${PORT} mode=${mode} CAPSOLVER_API_KEY=${hasCapsolverKey ? 'present' : 'absent'} SAT_VALIDATOR_SECRET=${hasSecret ? 'present' : 'absent'}`,
  )
})
